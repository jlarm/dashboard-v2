                ...
                if (isset($asset['flaws']) && is_array($asset['flaws'])) {
                    foreach ($asset['flaws'] as $flaw) {
                        $vulnerabilities[] = [
                            ...
                            'published_date' => isset($latestScan['scan_finished']) ? date('Y-m-d', strtotime((string) $latestScan['scan_finished']) ?: null) : '-',
                ...
                if (isset($asset['vulnerabilities']) && is_array($asset['vulnerabilities'])) {
                    foreach ($asset['vulnerabilities'] as $vuln) {
                        $vulnerabilities[] = [
                            ...
                            'published_date' => isset($vuln['firstSeen']) ? date('Y-m-d', strtotime((string) $vuln['firstSeen']) ?: null) : '-',
