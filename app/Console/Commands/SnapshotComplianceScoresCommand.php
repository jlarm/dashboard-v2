...
class SnapshotComplianceScoresCommand extends Command
{
    ...
    public function handle(CalculateComplianceScore $calculator): void
    {
        ...
        tenancy()->runForMultiple($tenants->isEmpty() ? null : $tenants, function () use ($calculator): void {
            $this->snapshotForCurrentTenant($calculator);
        });
    }

    private function snapshotForCurrentTenant(CalculateComplianceScore $calculator): void
    {
        $today = CarbonImmutable::now();

        Store::query()->each(function (Store $store) use ($calculator, $today): void {
