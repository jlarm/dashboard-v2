...
import { AlertOctagon, Download, FileSignature, Loader2, Trash2 } from 'lucide-vue-next';
...
const remove = (manual: Manual): void => {
    ...
    router.delete(redFlag.destroy.url({ manual: manual.id }), {
        preserveScroll: true,
        onFinish: () => {
...
<template>
    <Head title="Red Flag Manuals" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-5">
            <header class="flex flex-wrap items-start justify-between gap-3">
                <Heading
                    title="Red Flag Manuals"
                    :description="
                        store
                            ? `Signed Red Flag identity-theft prevention manuals for ${store.name}.`
                            : 'Signed Red Flag identity-theft prevention manuals across your stores.'
                    "
                />
                <Button v-if="store" as-child size="sm">
                    <Link :href="redFlag.create.url()">
                    ...
                    <TableBody>
                                ...
                                <TableCell class="w-0 whitespace-nowrap pr-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        >
                                            ...
                                            <DialogContent>
                                                <DialogHeader>
                                                    <DialogTitle>Delete Red Flag Manual</DialogTitle>
                                                </DialogHeader>
                                                <p class="text-sm text-muted-foreground">
                                                    Delete the Red Flag manual signed on
                        ...
                        <TableRow v-else>
                            <TableCell colspan="4" class="py-12 text-center">
                                <AlertOctagon class="mx-auto size-10 text-muted-foreground" />
