...
import { ClipboardList, Download, FileSignature, Loader2, Trash2 } from 'lucide-vue-next';
...
const remove = (manual: Manual): void => {
    ...
    router.delete(cms.destroy.url({ manual: manual.id }), {
        preserveScroll: true,
        onFinish: () => {
        ...
        <div class="space-y-5">
            <header class="flex flex-wrap items-start justify-between gap-3">
                <Heading
                    ...
                    :description="
                        store
                            ? `Signed Compliance Management System manuals for ${store.name}.`
                            : 'Signed Compliance Management System manuals across your stores.'
                    "
                />
                <Button v-if="store" as-child size="sm">
                    <Link :href="cms.create.url()">
                                ...
                                <TableCell class="w-0 whitespace-nowrap pr-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        >
                                            ...
                                            <DialogContent>
                                                <DialogHeader>
                                                    <DialogTitle>Delete CMS Manual</DialogTitle>
                                                </DialogHeader>
                                                <p class="text-sm text-muted-foreground">
                                                    Delete the CMS manual signed on
                                                    <strong>{{ manual.signed_at }}</strong>?
                                                    This permanently removes the PDF and signatures.
                                                </p>
                                                <DialogFooter>
