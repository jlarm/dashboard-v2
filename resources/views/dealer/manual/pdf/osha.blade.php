...
<!--  Front Page -->
<div class="cover">
    <img src="{{ global_asset('storage/logo.webp') }}" alt="">
    <h2 class="pdf-desc">OSHA General Safety Plans</h2>
    <p class="pdf-date">{{ $osha->created_at->format('F d, Y') }}</p>
</div>
<div class="footer">
...
<div>
    <p>Names, home phone number, and cell phone number for all key management personnel
    ...
    <p>Police department emergency number <strong>{{ $osha->police_emergency_phone }}</strong></p>
    <p>Police department non-emergency number <strong>{{ $osha->police_non_emergency_phone }}</strong></p>
    <p>What type of fire alarm System do you use? <strong>{{ $osha->fire_alarm_type }}</strong></p>
    <p>What type of burglar alarm system do you use? <strong>{{ $osha->burglar_alarm_type }}</strong></p>
    <br/><br/>
    <p><i>Signed off on {{ $osha->created_at->format('F d, Y') }} by {{ $osha->qualified_individual_name }}</i></p>
...
<div id="emergency-action-plan">
    ...
    <h3>Purpose</h3>
    <p><strong>{{ config('app.name') }}</strong> is dedicated to the protection of its employees from emergencies such
        as
    ...
    <p>OSHA's Emergency Action Plan requirements, found at 29 CFR 1910.38(a), require
        <strong>{{ config('app.name') }}</strong> to have a written emergency action plan (EAP). This plan applies to
        all operations
    ...
    <p><strong>{{ $osha->qualified_individual_name }}</strong> (or designee) is the (EAP) Emergency Action Plan
    ...
    <p><strong>{{ $osha->qualified_individual_name }}</strong> is responsible for reviewing and updating the plan as
        necessary. Copies
        of this plan may be obtained from <strong>{{ $osha->qualified_individual_name }}’s</strong> office or compliance
        dashboard.</p>
    <p><strong>{{ $osha->qualified_individual_name }}</strong>, has full authority to decide to implement the EAP if
        he/she believes an
    ...
    <p><strong>{{ $osha->qualified_individual_name }}</strong>, <strong>{{ $osha->service_manager_name }}</strong>, or
    ...
    <p>If, after reading this plan, you find that improvements can be made, please contact the Plan
        Administrator, <strong>{{ $osha->qualified_individual_name }}</strong>. We encourage all suggestions because we
        are committed
    ...
    <p>Different emergencies call for different alarms to indicate what actions employees should take.
        <strong>{{ config('app.name') }}</strong> has established an employee alarm system that complies with 29 CFR
        1910.165.
        <strong>{{ config('app.name') }}</strong> uses in house fire suppression... We use a distinctive alarm capable
        of
    ...
    <p>Some emergencies require evacuation or escape procedures, while some require employees to
        stay indoors, or in a safe area. Our emergency escape procedures are designed to respond to many potential
        emergencies, depending on the degree of seriousness. Nothing in these
    ...
    <p>Frontline supervisors must be aware of the locations of those employees working on a particular
        ...
        from the premises. Accounting for employees and non-employees will aid local responding fire/rescue departments
        in determining whether rescue efforts are necessary. Each manager will
    ...
    <ul>
        ...
        <li>Reports in to a central checkpoint managed by <strong>{{ $osha->qualified_individual_name }}</strong> or
            designee, and
        </li>
    ...
    <p><strong>{{ $osha->qualified_individual_name }}</strong> or designee will determine when it is safe for employees
        to leave their
    ...
    <p>During an emergency, <strong>{{ $osha->qualified_individual_name }}</strong> will do the following:</p>
...
<div id="hazard-communication-program">
    ...
    <p>The purpose of this program is to inform interested persons, including employees, that
        <strong>{{ config('app.name') }}</strong> is complying with the OSHA Hazard Communication Standard, Title 29
        Code of
    ...
    <p>The safety and health manager, <strong>{{ $osha->qualified_individual_name }}</strong>, is the program
        ...
        <strong>{{ $osha->qualified_individual_name }}</strong> will review and update the program, as necessary. Copies
        of the written program
        may be obtained from <strong>{{ $osha->qualified_individual_name }}</strong>, service department or online
        compliance
        dashboard.</p>
    <p>All employees, or their designated representatives, can obtain further information on this written
        ...
        from <strong>{{ $osha->qualified_individual_name }}’s</strong> office and dealership’s compliance dashboard.</p>
    ...
    <p>The safety and health manager, <strong>{{ $osha->qualified_individual_name }}</strong>, also keeps the chemical
        inventory list,
    ...
    <p><strong>{{ $osha->parts_manager_name }}</strong>, <strong>titlex3</strong> will be responsible for
    ...
    <p>The material safety data sheets are kept at the following location(s) in our facility: service
        department’s supervisor&#39;s office, <strong>{{ $osha->service_manager_name }}</strong>. Employees can obtain
        access to them by
    ...
    <p><strong>{{ $osha->service_manager_name }}</strong>, is responsible for ensuring that all hazardous chemicals in
        in-plant
    ...
    <p><strong>{{ $osha->parts_manager_name }}</strong>, is responsible for ensuring the proper labeling of any shipped
        containers.</p>
    <p><strong>{{ $osha->parts_manager_name }}</strong> or designee will refer to the corresponding SDS to assist
        employees in
...
<div id="hazwoper">
    ...
    <h3>Administrative Duties</h3>
    <p><strong>{{ $osha->service_manager_name }}</strong>, is responsible for developing and maintaining our facility&#39;s
        HAZWOPER
        plan. The plan is available for review and is kept at supervisor&#39;s and or manager&#39;s office in the
    ...
    <p><strong>{{ $osha->service_manager_name }}</strong>, is in charge of the HAZWOPER safety and health programs.</p>
    ...
    <p><strong>{{ $osha->service_manager_name }}</strong>, is the general supervisor who has the responsibility and
        authority to direct
        all hazardous waste operations.</p>
    <p><strong>{{ $osha->qualified_individual_name }}</strong>, is the site safety and health supervisor who has the
        responsibility and
    ...
    <p>Other personnel who are needed for hazardous waste site operations and emergency response
        are <strong>{{ $osha->service_manager_name }}</strong>, and <strong>{{ $osha->parts_manager_name }}</strong>.
        Their general functions and responsibilities are
    ...
    <p>The lines of authority, responsibility, and communication are as follows:
        ...
        <strong>{{ $osha->service_manager_name }}</strong> and then <strong>{{ $osha->parts_manager_name }}</strong></p>
...
<div id="lockout-tagout">
    ...
    <p>This procedure establishes this company&#39;s requirements for the lockout of energy isolating
        ...
        <strong>{{ config('app.name') }}</strong> where employees must deal with lockout/tagout situations as part of
        their job
    ...
    <p>Authorized employees subject to the requirements of this program and to be trained on their
        ...
        supervisory personnel only, no hourly employees from <strong>{{ config('app.name') }}</strong> are authorized to
        utilize
        lockout/tagout equipment. Affected employees subject to the requirements of this program and to be trained on
        their duties
    ...
    <p>Checking that no personnel are exposed, then
        ...
        Return the operating control(s) to neutral or &quot;off&quot; position after verifying that the equipment is
    ...
    <h3>Administrative Duties</h3>
    <p><strong>{{ $osha->service_manager_name }}</strong>, has overall responsibility for coordinating safety and health
        programs in this
        company.</p>
...
<div id="electrical-safety-plan">
    ...
    <p>The purpose of this program is to inform interested persons, including employees, that
        <strong>{{ config('app.name') }}</strong> is complying with the OSHA Electrical Safety Standard, Title 29 Code
        of Federal
        ...
        operations at <strong>{{ config('app.name') }}</strong> where employees may be exposed to live parts and/or
        those parts
        that have been deenergized.</p>
    <p><strong>{{ $osha->qualified_individual_name }}</strong> has overall responsibility for coordinating safety and
        health programs in
        this company. <strong>{{ $osha->service_manager_name }}</strong> is the person having overall responsibility for
        the Electrical
    ...
    <p>To determine areas of <strong>{{ config('app.name') }}</strong> that need to be included in the Electrical Safety
        Program,
    ...
    <p>Employees of our company who are qualified to work on, near, or with energized electric circuits
        ...
        of such training, has demonstrated an ability to perform duties safely at his or her level of training and who
        is under the direct supervision of a qualified person that is a qualified person
    ...
    <p>Every employee at <strong>{{ config('app.name') }}</strong> who faces the risk of electric shock from working on
        or near
    ...
    <p><strong>{{ config('app.name') }}</strong> conducts the electrical safety training for all employees. Every
        employee who
    ...
    <p>It is a <strong>{{ config('app.name') }}</strong> policy that circuits and equipment must be disconnected from
        all electric
    ...
    <ul>
        <li><strong>Application of locks and tags.</strong> Only authorized employees are allowed to place a lock and
        ...
        <li><strong>Verification of deenergized condition of circuits and equipment.</strong> Prior to work on the
            equipment, we require that a &quot;qualified&quot; employee verify that the equipment is deenergized and
            cannot be restarted.
        </li>
        <li><strong>Reenergizing circuits and equipment.</strong> Before circuits or equipment are reenergized, we
            follow these steps in this order:
            <ul>
                <li>A &quot;qualified&quot; employee conducts tests and verifies that all tools and devices have been
                    removed.
                </li>
    ...
    <p>In addition to lockout and tagging elements, see &quot;LOTO&quot; safety tab for additional full disclosure
        procedures.</p>
    ...
    <ul>
        ...
        <li>Battery Smart Module (BSM) which communicates on serial data to identifies a number of situations where high
            voltage should be disabled or any other number of issues identified i.e., shorts, SIR, interlocks.
            <ul>
                ...
                <li>Orange service plug (Disconnect switch) – for additional protection (interrupts the voltage coming
                    out of battery packs).
                </li>
            </ul>
        </li>
        <li>Once the vehicles Key is turned off, the BSM Opens the contacts inside the battery disconnecting the High
            Voltage source from the actual outlet terminals that provide power to the vehicle.
        </li>
    </ul>
    ...
    <ol>
        ...
        <li><span class="red">(Put High Voltage Gloves on)</span>
            <ul>
                <li>Disconnect the orange service plug on the side of the HV battery and remove and store it on the
                    workbench as well. This gives an additional level of protection in case the contact relays were
                    stuck “Closed” after turning the key off or disconnecting battery terminals.
                </li>
            </ul>
        </li>
        <li>Wait 10 minutes to ensure capacitors are fully discharged
            <ul>
                <li>Take your volt meter and put leads on the positive and negative battery terminals and make sure the
                    meter shows active 12 volts. This confirms your settings are correct on the meter.
                </li>
            </ul>
        </li>
        <li><span class="red">(HV Gloves on)</span>
            <ul>
                <li>Remove the Power Inverter electrical cover to give you access to the HV terminals inside.</li>
            </ul>
        </li>
        <li><span class="red">(HV Gloves on)</span>
            <ul>
                <li>Take the voltmeter and touch each HV terminal and check that it is registering “0” volts.</li>
            </ul>
        </li>
        <li><span class="red">(HV Gloves on)</span>
            <ul>
                <li>Reverse the polarity and check HV terminals again ensuring a reading of “0” volts.</li>
                <li>This will ensure that the vehicle has been disabled from all electrical charges throughout.</li>
            </ul>
        </li>
        <li>Re-check volt meter on the battery terminals again to ensure the meter is still operating properly and that
            all electrical tests were being recorded accurately at ZERO voltage. Doing this last check is called the
            “Live Dead Live” procedure.
        </li>
    </ol>
    <h3>Live Dead Live Procedure Completed</h3>
    <p><i>***Now you will know positively that the Hybrid/EV vehicle is not carrying any voltage that can harm you in
            any way. It is now safe to work on your vehicle.
        </i></p>
</div>
<div class="page-break"></div>
<div id="storm-water-pollution-plan">
    ...
    <h3>General Company Policy</h3>
    <p>The purpose of this program is to inform interested persons, including employees, that our company is complying
        with EPA requirements for preparing and maintaining a Stormwater Written Pollution Prevention Plan under the
        stormwater regulations (40 CFR 122).</p>
    <h3>Administrative Duties</h3>
    <p><strong>{{ $osha->service_manager_name }}</strong>, is responsible for our written stormwater pollution
        prevention plan. Copies of our written plan may be obtained from service departments office. Employees can
        obtain access to the plan by accessing the supervisor’s office area to review.
        <strong>{{ $osha->service_manager_name }}</strong> and <strong>{{ $osha->qualified_individual_name }}</strong>,
        make up our company's stormwater pollution prevention committee.</p>
    ...
    <p><strong>{{ config('app.name') }}</strong> has no significant materials that have been treated, stored or disposed
        in a manner to allow exposure to stormwater.</p>
    <p>At our facility, the following activities have a high potential for contaminating stormwater: working with the
        removal and storage of gasoline, oils, antifreeze etc. the following pollutants of concern may be associated
        with these activities: oil and gasoline discharge in excess amounts</p>
    <p>The following is the method we use for the on-site storage or disposal of these materials: 55-gallon drums and
        larger quantities kept outside above storage drums.</p>
    <p>We use the following materials management practices to minimize contact of these materials with precipitation and
        stormwater runoff: utilizing built in traps and have them cleaned out and removed by authorized Haz-Mat
        companies</p>
    <p>Our facility has the following materials loading and access areas: parts department loading dock area</p>
    <p>To help reduce pollutants in stormwater runoff, we use the following existing structural and non-structural
        control measures: utilizing built in traps and have them cleaned out and removed by authorized Haz-Mat companies
        and to include regularly scheduled actions such as sweeping or inspections. </p>
    <p>These measures are located throughout the dealership.</p>
    <p>Our facility's stormwater does receive treatment. The following process is used: local city and or county water
        treatment centers</p>
    <p>The following are predictions of the direction of flow for the types of pollutants likely to be present for each
        area of the plant generating stormwater discharges associated with industrial activity: N/A </p>
    <p>The following types of pollutants are likely to be present for each area of the plant generating stormwater
        discharges associated with industrial activity: oils, gasoline and antifreeze. </p>
    <p>The following is a summary of our existing stormwater sampling data: there are no sampling measures at this
        time.</p>
    <h3>Primary Stormwater Practices and Elements</h3>
    <p>We use standard and accepted practices, including management, maintenance, training, and inspections, to reduce
        pollutants.</p>
    <h3>Maintaining the Plan</h3>
    <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for conducting periodic site evaluations.</p>
    <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for keeping records of all inspections and
        reports.</p>
    ...
    <p><strong>{{ $osha->qualified_individual_name }}</strong> and or <strong>{{ $osha->service_manager_name }}</strong>
        is responsible for incorporating into the plan any necessary changes resulting from major changes in a
        facility's design, construction, operation or maintenance.</p>
    <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for determining if the facility is subject to
        any special requirements.</p>
    <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for incorporating into the plan any applicable
        special requirements.</p>
    <h3>Appendix</h3>
    <p>We have attached lists, samples, or procedures to this plan that will help ensure better understanding of our
        written program.</p>
</div>
<div class="page-break"></div>
<div id="used-oil-management">
    ...
    <p>Used oil, even when not classified as a hazardous waste under RCRA, can have harmful effects if it is released
        into the environment. In addition, people's health can be affected if used oil is handled improperly.</p>
    <p>Superfund regulations allow the federal government to hold any party that created or contributed to the creation
        of a hazardous waste site (including some used oil) financially responsible for cleanup costs.</p>
    <p>Used oil is a valuable resource because it has lubrication value and heat value. When treated to remove
        contaminants, the used oil can be used as a base stock to produce new lubricating oil. Because used oil has heat
        value, it can be burned as fuel. Properly burning the used oil keeps its heat value from being wasted and saves
        the virgin heating oil that would be burned instead.</p>
    <h3>Purpose</h3>
    <p>This plan provides one source of written documentation for used oil records for
        <strong>{{ config('app.name') }}</strong>. In addition, this plan will inform interested persons, including
        company and contractor employees, about this company's compliance with Environmental Protection Agency (EPA)
        requirements (found at 40 CFR 279) for used oil generators.</p>
    <p>This plan provides a written description of used oil management procedures, disposal methods, and transportation
        requirements. We encourage any suggestions that our employees have for improving our written plan for used oil
        management, as we are committed to developing and maintaining an effective protocol. We strive for clear
        understanding, environmentally sound practices, and involvement in the plan from every level of the company.</p>
    <h3>Administrative Duties</h3>
    <p>Service manager is responsible for developing the written used oil management plan; for ensuring that our written
        plan is complete, kept up to date, and made available to applicable or required authorities; and for maintaining
        used oil management records. A copy of our used oil management plan may be reviewed by employees. It is located
        in the service manager’s office.</p>
    <h3>Used Oil Defined</h3>
    <p>The EPA defines used oil as "any oil that has been refined from crude oil or any synthetic oil that has been used
        and as a result of such use is contaminated by physical or chemical impurities." Used oil can be generated
        during "do-it-yourself" projects, from automotive sources, or during industrial operations. This includes oils
        that are used as hydraulic fluid as well as oils that are used to lubricate automobiles and other machinery,
        cool engines, or suspend materials in industrial processes. Oils used for these purposes can become contaminated
        with physical materials (such as metal particles from engine wear) or chemical contaminants (such as gasoline
        combustion products, like toluene).</p>
    <h3>Used Oil Management</h3>
    <p>At this facility, we generate used oil from removal of oil from automobiles for oil change operations. It is
        likely to contain the following contaminants: metals and other engine related material</p>
    ...
    <ul>
        ...
        <li>Make sure our collection and storage set-ups are leak proof, spill proof, and that tanks have lids or are
            covered to prevent water from entering.
        </li>
        <li>Use lockable fills to prevent dumping of materials into the tank when it is not supervised.</li>
        <li>Maintain our collection containers regularly, comply with local fire and safety regulations, and avoid leaks
            and spills.
        </li>
        <li> Label storage tanks "Used Oil."</li>
        <li>Clean up any used oil spills or leaks. This includes providing soak-up material (e.g., sawdust, kitty
            litter, or a commercial product) for minor spills. It keeps the area clean and helps prevent personal
            injury.
        </li>
    ...
    <ul>
        <li>Our facility stores used oil in 55-gallon drums and above storage tanks. This facility follows these storage
            practices. We:
        </li>
        <li>Never mix used oil with any other material. This facility keeps gasoline, solvents, degreasers, paints, and
            so on, from making the used oil a hazardous waste and increasing collection costs.
        </li>
        <li>Carefully record the amount of used oil placed into and removed from storage devices. Recordkeeping plays an
            important role in leak detection for ASTs, USTs, and drums.
        </li>
        <li>Have constructed secondary containment around our drums/tanks with a capacity for 100 percent of the
            contents of the drums we store; the base of the containment area is sloped so that any spilled oil may be
            recollected and removed.
        </li>
        <li>Equip storage containers with wide-mouth, long-necked funnels to reduce spills during QUg.</li>
        <li>Equip storage containers with a pressure relief valve to reduce a buildup of pressure, which could cause
            leaks.
        </li>
    ...
    <ul>
        <li>Recycling used oil cashes in on either its lubricating value or heat value. We use this method of management
            whenever possible because it is easier to do and more cost effective than properly disposing of used oil.
        </li>
        <li>At this facility, we recycle our used oil by having it hauled away by a state approved used oil company
            method because we do not recycle it or use it for any other function on-site.
        </li>
    </ul>
    <p><i>Responding to Releases of Used Oil</i></p>
    <ul>
        <li>Even though all steps have been taken to prevent leaks or spills from occurring, this company is also
            prepared to respond to spills of used oil. We instruct workers to use the following protocol to manage
            spills of used oil and provide any necessary equipment:
        </li>
        <li> Stop the release. This action will vary depending on why the release is occurring. For example, if the
            spill occurs because a 55-gallon drum has been knocked over, the drum should be righted to stop more used
            oil from being released.
        </li>
        <li>If the spill occurs because a valve on a storage device has been left open, the valve should be closed. If a
            leak is a result of a puncture in the tank or drum, rags or similar materials should be used to plug the
            leak.
        </li>
        <li>Contain the release. We strive to prevent the used oil that has been released from spreading. For example, a
            sorbent, such as kitty litter or sawdust, should be spread over the spilled used oil.
        </li>
        <li>Clean up the release. Depending on the extent of the release, cleaning up the used oil can be a simple or a
            complicated task. For small spills on the ground, the soil can be dug up and disposed of. (The soil must be
            tested to determine if it exhibits hazardous characteristics.) For larger spills where puddles of used oil
            have formed, vacuum-type machinery can be used to collect the used oil before the soil is dug up for
            disposal. Because releases that contaminate a great deal of soil or ground or surface water are very
            difficult to clean up, this company contacts professionals to conduct the cleanup operation.
        </li>
        <li>Properly manage the used oil that has been cleaned up. Any leaked or spilled used oil is managed just like
            any other used oil under 40 CFR 279.
        </li>
        <li>Properly manage the solid materials generated during the cleanup. We place solid materials used to clean up
            a spill of used oil in a sieve-like container to allow the used oil to drip from the solid materials into a
            storage device. In addition, we compact the materials to remove the used oil. (Removal is complete when
            there are no more signs of free-flowing oil.) Materials contaminated with used oil that are burned can be
            managed in the same manner as used oil.
        </li>
        <li>Contaminated materials that will not be burned for energy are tested to determine if they exhibit hazardous
            waste characteristics. If they do not test hazardous, they are disposed of in a RCRA subtitle D facility. If
    ...
    <ul>
        <li>Although recycling is the best management option for used oil, this company has discovered this option is
            not feasible. Knowledge of how the used oil was generated or testing has revealed that our used oil is
            contaminated with none. This used oil is generated in quantities too small to be economically recycled.
            Therefore, we have chosen to properly dispose of our used oil.
        </li>
        <li>Because our used oil is contaminated with halogens in concentrations less than 1,000 ppm or not considered a
            hazardous waste, we place the used oil in a landfill that accepts industrial wastes and meets RCRA Subtitle
            D requirements.
        </li>
    </ul>
    <p><i>Managing and Disposing of Used Oil Filters</i></p>
    <ul>
        <li>Whenever this company changes the oil in a fleet vehicle, the oil filter is also changed to keep the solid
            contaminants of the old oil from immediately contaminating the new oil. Used oil filters can contain 10 to
            16 ounces of used oil; therefore, proper management of this source of used oil is a concern of this company.
            Used oil filters are not considered a hazardous waste under RCRA if they are not terne-plated and have been
            properly drained of oil.
        </li>
        <li>When used oil filters are removed from a warm engine, this company uses the Gravity draining - when the
            filter is removed from the engine, it should be placed with its gasket side down in a drain pan. If the
            filter has an anti-drain valve, the "dome end" of the filter should be punctured with a screwdriver (or
            similar device) so that the oil can flow freely. The filter should then be allowed to drain for 12 to 24
            hours.
        </li>
        <li>We store our drained used oil filters in a covered, rainproof container to prevent used oil from being
            washed from the filters to the surrounding environment. Our used oil filters are then recycled or properly
            disposed of.
        </li>
    </ul>
    <p><i>Shipping/Transporting Used Oil</i></p>
    <ul>
        <li>The used oil management standards define a used oil transporter as any person who transports used oil, any
            person who collects used oil from more than one generator and transports the collected oil, and owners and
            operators of used oil transfer facilities." <strong>{{ config('app.name') }}</strong> utilizes a waste
            transport company for hauling our used oil and other products for proper disposal. Our transporter has an
            EPA ID number and complies with all relevant used oil regulations, including keeping tracking records of
            where the used oil is collected and where it will be transported to.
        </li>
    </ul>
    ...
    <ul>
        ...
        <li>Measure the level of oil in a tank before and after the hauler collects it to be certain the oil collected
        ...
        <li>Ask for a receipt from the transporter that states how much used oil was collected from our facility and
            where the used oil will be taken. (These records are not required under the used oil management standards,
            but may be useful should a problem arise.)
        </li>
        <li>Make sure that the hauler maintains storage tanks/containers; labels containers "Used Oil"; stores used oil
            over oil-impervious surfaces; has secondary containment structures in place; stores used oil for no more
            than 35 days; tests waste in out-of-service tanks; closes out-of-service tanks containing hazardous waste
            according to EPA standards.
        </li>
    </ul>
    <h3>Employee Training</h3>
    <p>Although training is not strictly required under the regulations, we have designated a service manager to train
        personnel who will handle used oil. Direct any questions concerning used oil training to this person.</p>
    <p>Under this plan, employees are informed of used oil management procedures relevant to the positions in which they
        work. This training occurs both in the classroom and on the job.</p>
    <p>We keep records of job titles and written job descriptions for all positions related to used oil management and
        the names of employees QUg each job. We also keep records describing the type and amount of training
        provided.</p>
    <h3>Related Requirements</h3>
    <p>Related regulations that we must still comply with include:</p>
    <ul>
        <li>Spill Prevention Control and Countermeasures requirements (40 CFR 112) Please see related documents for
            information on how this company meets these additional requirements.
        </li>
    </ul>
    <h3>Recordkeeping</h3>
    ...
    <ul>
        ...
        <li>Updating the plan as needed by incorporating any necessary changes resulting from major changes in our
            facility's operation or maintenance.
        </li>
    </ul>
</div>
<div class="page-break"></div>
<div id="rpp">
    ...
    <h3>Purpose</h3>
    <p>This respirator program lays out standard operating procedures to ensure the protection of all employees from
        respiratory hazards in accordance with the requirements of OSHA 29 CFR 1910.134.</p>
    <h3>Administrative Duties</h3>
    <p>At <strong>{{ config('app.name') }}</strong> our Respiratory Protection Program Administrator is body
        <strong>{{ $osha->body_shop_manager_name }}</strong>.</p>
    <p>The body shop manager is also qualified by appropriate training and experience that is commensurate with the
        complexity of the program to administer or oversee our Respiratory Protection Program and conduct the required
        evaluations of program effectiveness.</p>
    <p>Employees may review a copy of our Respiratory Protection Program. It is located in the supervisor's office,
        <strong>{{ $osha->body_shop_manager_name }}</strong>'s. Our Program Administrator,
        <strong>{{ $osha->qualified_individual_name }}</strong>, reviews this program periodically to ensure its
        effectiveness.</p>
    <h3>Respirator Selection</h3>
    <p>Respirators are selected on the basis of respiratory hazards to which the worker is exposed and workplace and
        user factors that affect respirator performance and reliability. All selections are made by the body shop
        manager.</p>
    <p>The body shop manager will develop detailed written standard operating procedures governing the selection of
        respirators using the following guidelines: Select and provide respirators based on respiratory hazard(s) to
        which a worker is exposed and workplace and user factors that affect respirator performance and reliability.</p>
    <p>* Select a NIOSH-certified respirator. </p>
    <p>* Identify and evaluate the respiratory hazard(s) in the workplace, including a reasonable estimate of employee
        exposures to respiratory hazard(s) and an identification of the contaminant's chemical state and physical form.
        Consider the atmosphere to be IDLH,</p>
    <p><strong><i>Immediately Dangerous to Life and Health</i></strong>, if you cannot identify or reasonably estimate
        employee exposure.</p>
    <p>* Select respirators from a sufficient number of respirator models and sizes so that the respirator is acceptable
        to, and correctly fits, the user.</p>
    <p>When selecting respirators for IDLH atmospheres:</p>
    <p>* Provide these respirators: - A full facepiece pressure demand SCBA certified by NIOSH for a minimum service
        life of thirty minutes, or - A combination full facepiece pressure demand supplied-air respirator (SAR) with
        auxiliary self-contained air supply.</p>
    <p>* Provide respirators NIOSH-certified for escape from the atmosphere in which they will be used when they are
        used only for escape from IDLH atmospheres.</p>
    <p>* Consider all oxygen-deficient atmospheres to be IDLH. Exception: If you can demonstrate that, under all
        foreseeable conditions, the oxygen concentration can be maintained within the ranges specified in Table II of 29
        CFR 1910.134 (i.e., for the altitudes set out in the table), then any atmosphere-supplying respirator may be
        used.</p>
    <p>When selecting respirators for atmospheres that are not IDLH:</p>
    <p>* Provide a respirator that is adequate to protect the health of the employee and ensure compliance with all
        other OSHA statutory and regulatory requirements, under routine and reasonably foreseeable emergency
        situations.</p>
    ...
    <ul>
        ...
        <li>An air-purifying respirator, provided that: (1) The respirator is equipped with an end-of-service-life
            indicator (ESLI) certified by NIOSH for the contaminant; or (2) If there is no ESLI appropriate for
            conditions in our workplace, implement a change schedule for canisters and cartridges that is based on
            objective information or data that will ensure that canisters and cartridges are changed before the end of
            their service life. Describe in the respirator program the information and data relied upon and the basis
            for the canister and cartridge change schedule and the basis for reliance on the data.
        </li>
    </ul>
    ...
    <ul>
        ...
        <li>An air-purifying respirator equipped with a filter certified by NIOSH under 30 CFR part 11 as a high
            efficiency particulate air (HEPA) filter, or an air-purifying respirator equipped with a filter certified
            for particulates by NIOSH under 42 CFR 84, or
        </li>
        <li>For contaminants consisting primarily of particles with mass median aerodynamic diameters (MMAD) of at least
            2 micrometers, an air-purifying respirator equipped with any filter certified for particulates by NIOSH.
        </li>
    </ul>
    <p>Respirators are selected according to 29 FR 1910.134(d).</p>
    <p>For information regarding the types of respirators in use at this facility and their uses, see the table in the
        full plan.</p>
    <h3>Medical Evaluations</h3>
    <p>At <strong>{{ config('app.name') }}</strong>, persons will not be assigned to tasks requiring use of respirators
        nor fit tested unless it has been determined that they are physically able to perform the work and use the
        respirator.</p>
    <p>Medical evaluations are provided according to the requirements of 29 CFR 1910.134(e).</p>
    <p>Employees may contact their supervisor for a copy of their confidential medical evaluation or questionnaire.</p>
    ...
    <p>Employees must pass one of the fit test types that follow the protocols and procedures contained in 29 CFR
        1910.134 Appendix A.</p>
    ...
    <ol>
        ...
        <li>Prior to the selection process, the test subject shall be shown how to put on a respirator, how it should be
            positioned on the face, how to set strap tension and how to determine an acceptable fit. A mirror shall be
            available to assist the subject in evaluating the fit and positioning of the respirator. This instruction
            may not constitute the subject's formal training on respirator use, because it is only a review.
        </li>
        <li>The test subject shall be informed that he/she is being asked to select the respirator that provides the
        ...
        <li>The test subject shall be instructed to hold each chosen facepiece up to the face and eliminate those that
            obviously do not give an acceptable fit.
        </li>
        <li>The more acceptable facepieces are noted in case the one selected proves unacceptable; the most comfortable
            mask is donned and worn at least five minutes to assess comfort. Assistance in assessing comfort can be
            given by discussing the points in the following item A.6. If the test subject is not familiar with using a
            particular respirator, the test subject shall be directed to don the mask several times and to adjust the
            straps each time to become adept at setting proper tension on the straps.
        </li>
        <li>Assessment of comfort shall include a review of the following points with the test subject and allowing the
        ...
        <li>The test subject shall conduct a user seal check, either the negative and positive pressure seal checks
            ...
            pressure checks, the subject shall be told to seat the mask on the face by moving the head from side-to-side
            and up and down slowly while taking in a few slow deep breaths. Another facepiece shall be selected and
            retested if the test subject fails the user seal check tests.
        </li>
        <li>The test shall not be conducted if there is any hair growth between the skin and the facepiece sealing
        ...
        <li>If a test subject exhibits difficulty in breathing during the tests, she or he shall be referred to a
        ...
        <li>Exercise regimen. Prior to the commencement of the fit test, the test subject shall be given a description
            of the fit test and the test subject's responsibilities during the test procedure. The description of the
            process shall include a description of the test exercises that the subject will be performing. The
            respirator to be tested shall be worn for at least 5 minutes before the start of the fit test.
        </li>
        <li>The fit test shall be performed while the test subject is wearing any applicable safety equipment that may
            be worn during actual respirator use which could interfere with respirator fit.
        </li>
        <li>Test Exercises. (a) The following test exercises are to be performed for all fit testing methods prescribed
            in this appendix, except for the CNP method. A separate fit testing exercise regimen is contained in the CNP
            protocol. The test subject shall perform exercises, in the test environment, in the following manner:
            <ul>
                <li>(1) Normal breathing. In a normal standing position, without talking, the subject shall breathe
                    normally.
                </li>
                <li>(2) Deep breathing. In a normal standing position, the subject shall breathe slowly and deeply,
                    taking caution so as not to hyperventilate.
                </li>
                <li>(3) Turning head side to side. Standing in place, the subject shall slowly turn his/her head from
                ...
                <li>(4) Moving your head up and down. Standing in place, the subject shall slowly move his/her head up
                    and down. The subject shall be instructed to inhale in the up position (i.e., when looking toward
                    the ceiling).
                </li>
                <li>(5) Talking. The subject shall talk out loud slowly and loud enough so as to be heard clearly by the
                    test conductor. The subject can read from a prepared text such as the Rainbow Passage, count
                    backward from 100, or recite a memorized poem or song. Rainbow Passage When the sunlight strikes
                    raindrops in the air, they act like a prism and form a rainbow. The
                ...
                <li>(6) Grimace. The test subject shall grimace by smiling or frowning. (This applies only to QNFT
                    testing; it is not performed for QLFT.)
                </li>
                <li>(7) Bending over. The test subject shall bend at the waist as if he/she were to touch his/her toes.
                ...
                <li>(b) Each test exercise shall be performed for one minute except for the grimace exercise which
    ...
    <p>Once the respirator has been properly selected and fitted, its protection efficiency must be maintained for
        proper use according to 29 CFR 1910.134(g). Our company ensures with written procedures that respirators are
        used properly in the workplace. Our proper respirator use procedures are:</p>
    ...
    <ul>
        <li>Facial hair that comes between the sealing surface of the facepiece and the face or that interferes with
            valve function; or
        </li>
        <li>Any condition that interferes with the face-to-facepiece seal or valve function.</li>
    </ul>
    <p>* If an employee wears corrective glasses or goggles or other personal protective equipment, ensure that such
        equipment is worn in a manner that does not interfere with the seal of the facepiece to the face of the
        user.</p>
    <p>* For all tight-fitting respirators, ensure that employees perform a user seal check each time they put on the
        respirator using the procedures in 29 CFR 1910.134 Appendix B-1 (User Seal Check Procedures) or procedures
        recommended by the respirator manufacturer that you can demonstrate are as effective as those in Appendix
        B-1.</p>
    <p>Continuing Respirator Effectiveness</p>
    <p>* Appropriate surveillance must be maintained of work area conditions and degree of employee exposure or stress.
        When there is a change in work area conditions or degree of employee exposure or stress that may affect
        respirator effectiveness, reevaluate the continued effectiveness of the respirator.</p>
    <p>* Ensure that employees leave the respirator use area:</p>
    <ul>
        <li>To wash their faces and respirator facepieces as necessary to prevent eye or skin irritation associated with
            respirator use; or - If they detect vapor or gas breakthrough, changes in breathing resistance, or leakage
            of the facepiece; or
        </li>
        <li>To replace the respirator or the filter, cartridge, or canister elements.</li>
        <li>If the employee detects vapor or gas breakthrough, changes in breathing resistance, or leakage of the
            facepiece, replace or repair the respirator before allowing the employee to return to the work area.
        </li>
    </ul>
    ...
    <p>* Visual, voice, or signal line communication is maintained between the employee(s) in the IDLH atmosphere and
        the employee(s) located outside the IDLH atmosphere;</p>
    <p>* The employee(s) located outside the IDLH atmosphere are trained and equipped to provide effective emergency
        rescue;</p>
    <p>* The employer or designee is notified before the employee(s) located outside the IDLH atmosphere enter the IDLH
        atmosphere to provide emergency rescue;</p>
    <p>* The employer or designee authorized to do so by the company, once notified, provides necessary assistance
    ...
    <ul>
        ...
        <li>Appropriate retrieval equipment for removing the employee(s) who enter(s) these hazardous atmospheres where
            retrieval equipment would contribute to the rescue of the employee(s) and would not increase the overall
            risk resulting from entry; or
        </li>
        <li>Equivalent means for rescue where retrieval equipment is not required under the bullet item above this
            one.
        </li>
    </ul>
    <p>Procedures for Interior Structural Firefighting</p>
    <p>In addition to the requirements set forth in the row above for Procedures for IDLH Atmospheres, in interior
        structural fires, ensure that:</p>
    <p>* At least two employees enter the IDLH atmosphere and remain in visual or voice contact with one another at all
        times;</p>
    <p>* At least two employees are located outside the IDLH atmosphere</p>
    <h3>Maintenance and Care Procedures</h3>
    <p>Our company provides each respirator user with a respirator that is clean, sanitary, and in good working order,
        according to the requirements in 29 CFR 1910.134(h)(1). We have created the following cleaning schedule(s) to be
        used for each respirator: Respirator issued for the exclusive use of an employee must be cleaned and disinfected
        as often as necessary to be maintained in a sanitary condition.</p>
    <p>* Respirators issued to more than one employee must be cleaned and disinfected before being worn by different
        individuals.</p>
    ...
    <p>We ensure that respirators are stored according to 29 CFR 1910.134(h)(2). They are packed or stored in employees’
        lockers to prevent deformation of the facepiece and exhalation valve.</p>
    <p>We inspect respirators according to the requirements in 29 CFR 1910.134(h)(3). We have created the following
        inspection schedule(s) to be used for each respirator: All types of respirators used in routine situations must
        be inspected before each use and during cleaning.</p>
    <p>* Respirators maintained for use in emergency situations must be inspected at least monthly and in accordance
        with the manufacturer's recommendations, and checked for proper function before and after each use.</p>
    <p>* Emergency escape-only respirators must be inspected before being carried into the workplace for use.</p>
    <p>See the attached respirator inspection records.</p>
    <p>Respirators that fail an inspection or are otherwise found to be defective are removed from service and are
        discarded or repaired or adjusted according to the procedures in 29 CFR 1910.134(h)(4).</p>
    ...
    <p>The following detailed procedures ensure adequate air quality, quantity, and flow of breathing air for
        atmosphere-supplying respirators and include coverage of the OSHA requirements for breathing air quality and use
        in 29 CFR 1910.134(I): N/A.</p>
    <h3>Training</h3>
    ...
    <p>At <strong>{{ config('app.name') }}</strong>, program evaluation is performed annually by our body shop manager
        according to 29 CFR 1910.134(l).</p>
    <h3>Appendices</h3>
...
<div id="bloodborne-pathogens">
    ...
    <p>Our exposure control plan (ECP) is provided to eliminate or minimize occupational exposure to bloodborne
        ...
        Pathogens."</p>
    <h3>Administrative Duties</h3>
    <p><strong>{{ $osha->qualified_individual_name }}</strong>, is responsible for the implementation of the ECP and is
        responsible for maintaining, reviewing, and updating the ECP at least annually.
        <strong>{{ $osha->qualified_individual_name }}</strong> is responsible for maintaining and providing all
        necessary equipment and supplies and also responsible for ensuring that all medical actions are performed and
        for maintaining appropriate records. <strong>{{ $osha->qualified_individual_name }}</strong> is responsible for
        training and allowing ECP access for appropriate personnel.</p>
    <h3>Methods of Implementation and Control</h3>
    <p>The plan indicates job classifications in which some employees at our establishment have occupational exposure.
        Universal Precautions All employees utilize universal precautions</p>
    <p><i>Exposure Control Plan </i></p>
    <p>Covered employees receive an explanation of this ECP during their initial training, and annually. Employees may
        review this plan at any time during their work shifts by contacting
        <strong>{{ $osha->qualified_individual_name }}</strong>. The plan indicates which devices have been identified
        as candidates for our use. <strong>{{ $osha->qualified_individual_name }}</strong> solicits input from
        non-managerial employees responsible for direct patient care in the identification, evaluation, and selection of
        effective engineering and work practice controls. <strong>{{ $osha->qualified_individual_name }}</strong>
        documents all solicitation in the ECP and indicates the engineering and work practice controls identified during
        solicitation in our annual reviews.</p>
    <p><i>Engineering and Work Practice Controls</i></p>
    <p>The plan indicates which specific engineering controls and work practice controls are used. The controls include
        how sharps disposal containers are maintained. The plan also indicates how we identify the need for changes in
        engineering control and work practices, and evaluates the need for new procedures or new products.
        <strong>{{ $osha->qualified_individual_name }}</strong> is responsible for the effective implementation controls
        recommendations.</p>
    <p><i>Personal Protective Equipment (PPE)</i></p>
    <p>PPE is provided to our employees at no cost to them. <strong>{{ $osha->qualified_individual_name }}</strong>
        provides appropriate PPE training.</p>
    ...
    <p>The plan indicates which labeling method(s) is used in this facility.
        <strong>{{ $osha->service_manager_name }}</strong>, ensures that proper, effective labeling is used. The local
        health department and <strong>{{ config('app.name') }}</strong> provides training to employees on hepatitis B
        vaccinations, addressing the safety, benefits, efficacy, methods of administration, and availability. Personal
    ...
    <p>Should an exposure incident occur, employees are to contact
        <strong>{{ $osha->qualified_individual_name }}</strong>. We provide immediate, confidential medical evaluation
        and follow-up after initial first aid.</p>
    <p><i>Administration of Post-Exposure Evaluation and Follow-up</i></p>
    <p><strong>{{ $osha->qualified_individual_name }}</strong> ensures that the responsible health care professional(s)
        is given a copy of OSHA's bloodborne pathogens standard, and
        <strong>{{ $osha->qualified_individual_name }}</strong> provides the employee with a copy of the evaluating
        health care professional's written opinion.</p>
    <p><i>Procedures for Evaluating the Circumstances Surrounding an Exposure Incident</i></p>
    <p><strong>{{ $osha->qualified_individual_name }}</strong> reviews the circumstances of all exposure incidents. If
        necessary, <strong>{{ $osha->qualified_individual_name }}</strong> ensures that appropriate revisions are made
        to this ECP.</p>
    <h3>Employee Training</h3>
    <p><strong>{{ $osha->qualified_individual_name }}</strong> conducts training for employees who have occupational
        exposure to bloodborne pathogens. The plan indicates what topics are covered in the training, and where training
        materials.</p>
    <h3>Recordkeeping</h3>
    <p><i>Training Records</i></p>
    <p>Training records are kept for at least three years at <strong>{{ $osha->qualified_individual_name }}</strong>
        office location. The plan describes which training records are included.</p>
    <p><i>Medical Records</i></p>
    <p><strong>{{ $osha->qualified_individual_name }}</strong> is responsible for maintaining the required medical
        records. These records are kept in the human resource office for at least the duration of employment plus 30
        years.</p>
    <p><i>OSHA Recordkeeping</i></p>
    <p>If an exposure incident occurs, it is evaluated, and <strong>{{ $osha->qualified_individual_name }}</strong>
        determines if the case meets OSHA's Recordkeeping Requirements.</p>
</div>
<div class="page-break"></div>
<div id="ppe">
    <h1 class="text-center">{{ config('app.name') }}</h1>
    ...
    <h3>Purpose</h3>
    <p>The purpose of this Personal Protective Equipment (PPE) Program is to document the hazard assessment, protective
        measures in place, and PPE in use at this company.</p>
    <p>While OSHA's Personal Protective Equipment regulation, found at 29 CFR 1910.132-.140 (Subpart I), does not
        explicitly require a written Personal Protective Equipment (PPE) Program,
        <strong>{{ config('app.name') }}</strong> has developed a written PPE program to document and specify all
        information relative to our PPE needs.</p>
    <p>The <strong>{{ $osha->service_manager_name }}</strong>, is the program coordinator, acting as the representative
        of the plant manager, who has overall responsibility for the program. The service manager will designate
        appropriate dealership supervisors to assist in training employees and monitoring their use of PPE. This written
        plan is kept in service department office will review and update the program as necessary. Copies of this
        program may be obtained from the service manager’s office.</p>
    <h3>Hazard Assessment</h3>
    <p>In order to assess the need for PPE, the Safety Manager, with other appropriate managers’ and employees will
        help:</p>
    <ul>
        <li>identifies job classifications where exposures occur or could occur, and</li>
        <li>conducts a walk-through survey of workplace areas where hazards exist or may exist to identify sources of
            hazards to employees.
        </li>
    </ul>
    <p>During the walk-through survey the Safety Manager observes and records the hazards along with PPE currently in
        use (type and purpose). The Safety Manager organizes the data and information for use in the assessment of
        hazards to analyze the hazards and enable proper selection of protective equipment.</p>
    <p>The Safety Manager documents the hazard assessment according to 29 CFR 1910.132(d)(2).</p>
    <h3>Selection Guidelines</h3>
    <p>Once any hazards have been identified and evaluated through hazard assessment, protective equipment is chosen
        according to the selection guidelines in Appendix B to Subpart I of 29 CFR 1910.</p>
    <p>It is the responsibility of the Safety Manager to reassess the workplace hazard situation as necessary, to
        identify and evaluate new equipment and processes, to review accident records, and reevaluate the suitability of
        previously selected PPE. This reassessment will take place as needed, but at least every quarter during
        inspection of the facility and work environment.</p>
    <h3>Employee Training</h3>
    <p>The Safety Manager/supervisor provides training, according to 29 CFR 1910.132(f), for each employee who is
        required to use personal protective equipment.</p>
    <p>Because failure to comply with company policy concerning PPE can result in OSHA citations and fines as well as
        employee injury, an employee who does not comply with this program will be disciplined for noncompliance
        according to the following schedule: employee will be subject to disciplinary action up to and including
        termination as determined by the shop foreman and service manager.</p>
    <h3>Cleaning and Maintenance</h3>
    <p>PPE is to be inspected, cleaned, and maintained by employees at regular intervals as part of their normal job
        duties so that the PPE provides the requisite protection.</p>
    ...
    <p><span class="italic underline">Eye Protection</span> - Work Area: service bay areas and parts department.</p>
    <p><span class="italic underline">Safety Shields / Goggles</span> - Work Area Hazard: grinding, welding flash,
        chemical splash, high temperature, splash from molten metal’s, hammering, metal fragments.</p>
    <p>Type of goggles/face shield: covered goggles, direct ventilation, face shields, welding helmets.</p>
    <p>Employees from temporary work agencies and contractors are required to wear goggles/face shields if assigned to
    ...
    <p>Type of safety shoe: Rubber heeled boots with Nitrile or Neoprene boots soles to protect against slipping on wet
        or oily floors. Also protects against chemical exposure. Also will have steel-toed shoes to protect against
        falling objects.</p>
    <p>Employees from temporary work agencies and contractors are required to wear safety shoes if assigned to work in
        the designated work areas.</p>
    <p>Members of the Emergency Response Team are required to wear safety footwear when responding to fire emergency
        situations.</p>
    <p>All employees who work in designated work areas and/or job assignments are responsible for purchasing and wearing
        safety shoes to comply with this policy. Purchase of shoes is done by Employees needing to purchase their own
        safety shoes through a local merchant as a condition of employment.</p>
    <h3>PPE Specific Information</h3>
    ...
    <p>Work area: service technicians and parts department employees.</p>
    <p>Hazard: working in and around car repair, moving boxes and other parts around in the parts department, utilizing
        gloves for welding.</p>
    ...
    <p>Employees from temporary work agencies and contractors are required to wear protective gloves if Employees in the
        following designated work areas are required to wear protective gloves:</p>
    <p>Work area: service technicians and parts department employees. </p>
    <p>Hazard: working in and around car repair, moving boxes and other parts around in the parts department, utilizing
        gloves for welding. </p>
    <p>Type of glove: gloves will be used and assessed based on type of job function being employed. Safety suppliers
        can help examine the attributes of the various types of gloves and advise accordingly.</p>
    <p>Employees from temporary work agencies and contractors are required to wear protective gloves if assigned to work
    ...
    <p>Work area: service technicians and parts department employees. </p>
    <p>Hazard: hair from oil, grease, water, dirt and sparks protect the head from hot exhaust systems, hot oil, sharp
        protruding objects and falling pipes, mufflers or other vehicle parts. They easily adapt for the wearing of eye
        goggles and safety glasses. Parts department employees can have parts and equipment fall off storage shelves
        when removing or storing such items.</p>
    <p>Type of hard hat: bump caps which will protect the employees from the items mentioned above.</p>
    <p>Employees from temporary work agencies and contractors are required to wear hard hats if assigned to work in the
        designated work areas.</p>
</div>
<div class="page-break"></div>
<div id="compressed-gas">
    ...
    <h3>Purpose</h3>
    <p>It is the policy of <strong>{{ config('app.name') }}</strong> to permit only trained and authorized employees to
        handle, store,
    ...
    <p><span class="italic underline">Handling Procedures</span>- We follow the safe handling procedures found in the
        CGA pamphlet
        series, including the P-1-1991 pamphlet.</p>
    <p><span class="italic underline">Storage Procedures</span> - We follow the safe storage procedures found in the CGA
        pamphlet series,
        including the P-1-1991 pamphlet.</p>
    <p><span class="italic underline">Usage Procedures</span> - We follow the safe storage procedures found in the CGA
        pamphlet series,
        including the P-1-1991 pamphlet.</p>
    <h3>Training Program</h3>
...
<div id="welding-and-cutting">
    ...
    <h3>Training</h3>
    <p>It is the policy of <strong>{{ config('app.name') }}</strong> to permit only trained and authorized personnel to
        operate
    ...
    <h3>Maintenance</h3>
    <p><strong>{{ $osha->parts_manager_name }}</strong>, <strong>titlex3</strong> complete(s) a receiving or delivery
    ...
    <p>(c)(1)(iv) Precautionary labels. A number of potentially hazardous materials are employed in fluxes, coatings,
        coverings, and filler metals used in welding and cutting or are released to the
    ...
    <h3>Recordkeeping</h3>
    <p><strong>{{ $osha->service_manager_name }}</strong>, <strong>titlex2</strong> is responsible for maintaining the
        following records: any service or
...
<div id="fpp">
    ...
    <p><strong>{{ $osha->qualified_individual_name }}</strong>, is the Plan Coordinator, acting as the representative of
        the Facility
        Manager, who has overall responsibility for the plan. The written plan is kept in Qualified
        Individual’s office and or online compliance dashboard. <strong>{{ $osha->qualified_individual_name }}</strong>,
        will review and
    ...
    <p>The FPP communicates to employees, policies and procedures to follow when fires erupt. This
        ...
        OSHA officials who ask to see it.</p>
    <p>If after reading this plan, you find that improvements can be made, please contact the Plan
        Coordinator, <strong>{{ $osha->qualified_individual_name }}</strong>. We encourage all suggestions because we
        are committed
    ...
    <p>The service and parts manager along with, <strong>{{ $osha->qualified_individual_name }}</strong>, will be
        responsible for the
    ...
    <p><strong><i>At this location the use of ABC class fire extinguishers are utilized along with class A</i></strong>
    </p>
...
<div id="machine-equipment">
    ...
    <ul>
        ...
        <li>Demonstrate <strong>{{ config('app.name') }}&#39;s</strong> compliance with machine safety and equipment
            usage
            requirements for general industry in Subpart O and P of 29 CFR 1910.
        </li>
    </ul>
    <h3>Administrative Duties</h3>
    <p><strong>{{ $osha->service_manager_name }}</strong>, our company&#39;s <strong>titlex2</strong>, is responsible
    ...
    <p><strong>{{ $osha->service_manager_name }}</strong>, and assigned supervisors are accountable for all phases of
        installation and
    ...
    <p>Although we may not be able to eliminate all problems, we try to eliminate as many as possible
        ...
        <strong>{{ $osha->service_manager_name }}</strong>, is responsible for evaluating and updating this written
        plan. The evaluation will
