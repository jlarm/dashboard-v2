...
<div class="w-full p-5">
    ...
    <div class="space-y-10 prose max-w-none px-6">
        <article id="eap">
            ...
            <div>
                <h2>Purpose</h2>
                <p><strong>{{ tenant('name') }}</strong> is dedicated to the protection of its employees
                ...
                <p>OSHA's Emergency Action Plan requirements, found at 29 CFR 1910.38(a), require
                    <strong>{{ tenant('name') }}</strong> to have a written emergency action plan (EAP).
                ...
                <p><strong>{{ $osha->qualified_individual_name }}</strong> (or designee) is the (EAP) Emergency Action
                    Plan administrator,
                    who has
                ...
                <ul>
                    ...
                    <li>Ensuring that equipment is placed and locked in storage rooms or desks for protection;
                    </li>
                ...
                <p><strong>{{ $osha->qualified_individual_name }}</strong> is responsible for reviewing and updating the
                    plan as necessary.
                    Copies
                    of this plan may be obtained from <strong>{{ $osha->qualified_individual_name }}’s</strong> office
                    or compliance
                    dashboard.</p>
                <p><strong>{{ $osha->qualified_individual_name }}</strong>, has full authority to decide to implement
                    the EAP if he/she
                    believes an
                ...
                <p><strong>{{ $osha->qualified_individual_name }}</strong>,
                    <strong>{{ $osha->service_manager_name }}</strong>, or
                    <strong>{{ $osha->parts_manager_name }}</strong></p>
                ...
                <table>
                    ...
                    <tbody>
                    <tr>
                        <td>{{ $osha->owner_name }}</td>
                        <td>Owner</td>
                        <td>{{ $osha->owner_phone }}</td>
                    </tr>
                    <tr>
                        <td>{{ $osha->general_manager_name }}</td>
                        <td>General Manager</td>
                        <td>{{ $osha->general_manager_phone }}</td>
                    </tr>
                    <tr>
                        <td>{{ $osha->service_manager_name }}</td>
                        <td>Service Manager</td>
                        <td>{{ $osha->service_manager_phone }}</td>
                    </tr>
                    <tr>
                        <td>{{ $osha->parts_manager_name }}</td>
                        <td>Parts Manager</td>
                        <td>{{ $osha->parts_manager_phone }}</td>
                    </tr>
                    <tr>
                        <td>{{ $osha->body_shop_manager_name }}</td>
                        <td>Body Shop Manager</td>
                        <td>{{ $osha->body_shop_manager_phone }}</td>
                    </tr>
                    </tbody>
                </table>
                ...
                <p>If, after reading this plan, you find that improvements can be made, please contact the Plan
                    Administrator, <strong>{{ $osha->qualified_individual_name }}</strong>. We encourage all suggestions
                    because we are
                    committed
                ...
                <p>Different emergencies call for different alarms to indicate what actions employees should
                    take.
                    <strong>{{ tenant('name') }}</strong> has established an employee alarm system that
                    complies with 29 CFR 1910.165.
                    <strong>{{ tenant('name') }}</strong> uses in house fire suppression... We use a
                    distinctive alarm capable of
                    identification as a signal whether or not to evacuate for each emergency. We realize that
                    where
                    ...
                    members,
                    if applicable. Fire alarms are located on each floor near each required exit. We will use
                    the
                ...
                <p>When employees detect an emergency that requires an evacuation, such as a fire or hazardous
                    release, they should notify direct supervisor, manager in charge of their department, call
                    911.
                ...
                <p>Supervisors will assist in a safe and orderly evacuation for all types of emergencies that
                    require
                ...
                <p>Frontline supervisors must be aware of the locations of those employees working on a
                    ...
                    fire/rescue departments in determining whether rescue efforts are necessary. Each manager
                    will
                ...
                <ul>
                    ...
                    <li>Reports in to a central checkpoint managed by
                        <strong>{{ $osha->qualified_individual_name }}</strong> or designee,
                        and
                    </li>
                    <li>Assumes role of department contact to answer questions.</li>
                </ul>
                <p>Head count results should be given to the Fire Chief, and firefighter, if requested. Follow
                    any
                ...
                <p>In the event of a tornado, it is corporate policy to provide emergency warning and shelter.
                    Once
                ...
                <p>Employees should stay away from windows, but stay inside the building they are in. Employees
                    ...
                    given.</p>
                <p><strong>{{ $osha->qualified_individual_name }}</strong> or designee will determine when it is safe
                ...
                <p>If anyone is injured or contaminated, the Plan Administrator will activate rescue and first
                    aid
                    actions coordinating with local emergency response teams.</p>
                <h2>Plan Administrator Duties</h2>
                <p>During an emergency, <strong>{{ $osha->qualified_individual_name }}</strong> will do the following:
                </p>
                ...
                <h3>Training</h3>
                <p>Our Plan Administrator reviews with each of our employees at the following times, those parts
                    of
        ...
        <article id="hcp">
            ...
            <p>The purpose of this program is to inform interested persons, including employees, that
                <strong>{{ tenant('name') }}</strong> is complying with the OSHA Hazard Communication
            ...
            <p>The safety and health manager, <strong>{{ $osha->qualified_individual_name }}</strong>, is the program
                coordinator, acting as
                the
                representative of the plant manager, who has overall responsibility for the program.
                <strong>{{ $osha->qualified_individual_name }}</strong> will review and update the program, as
                necessary. Copies of the
                written program
                may be obtained from <strong>{{ $osha->qualified_individual_name }}</strong>, service department or
                online compliance
                dashboard.</p>
            <p>All employees, or their designated representatives, can obtain further information on this
                ...
                from <strong>{{ $osha->qualified_individual_name }}’s</strong> office and dealership’s compliance
                dashboard.</p>
            ...
            <p>Our chemical inventory is a list of hazardous chemicals known to be present in our workplace.
                ...
                kept.</p>
            <p>The safety and health manager, <strong>{{ $osha->qualified_individual_name }}</strong>, also keeps the
                chemical inventory
                list,
            ...
            <p>After the chemical inventory is compiled, it serves as a list of every chemical for which an SDS
                must be maintained. Parts <strong>Manager</strong> updates the inventory as necessary.</p>
            <h2>Material Safety Data Sheets (SDS’s)</h2>
            <p><strong>{{ $osha->parts_manager_name }}</strong>, <strong>Parts Manager</strong> will be responsible for
                ...
                new procurements for the company must be cleared by <strong>{{ $osha->parts_manager_name }}</strong> or
                <strong>{{ $osha->service_manager_name }}</strong>,</p>
            <p>The material safety data sheets are kept at the following location(s) in our facility: service
                department’s supervisor&#39;s office, <strong>{{ $osha->service_manager_name }}</strong>. Employees can
                obtain access to
                them by
            ...
            <p><strong>{{ $osha->service_manager_name }}</strong>, is responsible for ensuring that all hazardous
                chemicals in in-plant
            ...
            <p><strong>{{ $osha->parts_manager_name }}</strong>, is responsible for ensuring the proper labeling of any
                shipped
                containers.</p>
            <p><strong>{{ $osha->parts_manager_name }}</strong> or designee will refer to the corresponding SDS to
                assist employees in
            ...
            <p>We ask our employees to ask Automotive Risk Management Partners and
                <strong>{{ $osha->service_manager_name }}</strong> if
            ...
            <p>All employees, or their designated representatives, can obtain further information on this
                ...
                from Automotive Risk Management Partners or <strong>{{ $osha->service_manager_name }}</strong>.</p>
            <h2>Appendix</h2>
        ...
        <article id="hazwoper">
            ...
            <h2>Administrative Duties</h2>
            <p><strong>{{ $osha->service_manager_name }}</strong>, is responsible for developing and maintaining our
                facility&#39;s
                HAZWOPER
            ...
            <p><strong>{{ $osha->service_manager_name }}</strong>, is in charge of the HAZWOPER safety and health
                programs.</p>
            ...
            <p>Workers also must know the names of personnel and their alternatives responsible for site
                ...
                they have been trained to the level required by their job function and responsibility and has
                been
            ...
            <p>At our facility, <strong>{{ $osha->qualified_individual_name }}</strong>, is the person responsible for
                conducting training.
                We make
            ...
            <p>We want to offer the best training for our workers, so the format of the program that is used is
                to
            ...
            <p><strong>{{ $osha->service_manager_name }}</strong>, is the general supervisor who has the responsibility
                and authority to
                direct
                all hazardous waste operations.</p>
            <p><strong>{{ $osha->qualified_individual_name }}</strong>, is the site safety and health supervisor who has
                the responsibility
                and
            ...
            <p>Other personnel who are needed for hazardous waste site operations and emergency response
                are <strong>{{ $osha->service_manager_name }}</strong>, and
                <strong>{{ $osha->parts_manager_name }}</strong>. Their general functions and
            ...
            <p>The lines of authority, responsibility, and communication are as follows:
                <strong>{{ $osha->qualified_individual_name }}</strong>,
                <strong>{{ $osha->service_manager_name }}</strong> and then
                <strong>{{ $osha->parts_manager_name }}</strong></p>
        ...
        <article id="lt">
            ...
            <p>This procedure establishes this company&#39;s requirements for the lockout of energy isolating
                ...
                <strong>{{ tenant('name') }}</strong> where employees must deal with lockout/tagout
            ...
            <p>Authorized employees subject to the requirements of this program and to be trained on their
                ...
                supervisory personnel only, no hourly employees from <strong>{{ tenant('name') }}</strong>
                are authorized to utilize
                lockout/tagout equipment. Affected employees subject to the requirements of this program and to
            ...
            <p>Verification of isolation. Prior to starting work on machines or equipment that have been locked
                out or tagged out, the authorized personnel shall verify that isolation and DE-energization of
                the
            ...
            <p>Lockout or tagout devices removal. Each lockout or tagout device shall be removed from each
                ...
                person who applied the lockout or tagout device is not available to remove it, that device may
                be
            ...
            <h2>Administrative Duties</h2>
            <p><strong>{{ $osha->service_manager_name }}</strong>, has overall responsibility for coordinating safety
                and health
                programs in this
                company.</p>
        ...
        <article id="esp">
            ...
            <p>The purpose of this program is to inform interested persons, including employees, that
                <strong>{{ tenant('name') }}</strong> is complying with the OSHA Electrical Safety Standard,
                Title 29 Code of Federal
                Regulations 1910.333, by determining that this workplace needs written procedures for
                preventing electric shock or other injuries resulting from direct/indirect electrical contacts
                to
                employees working on or near energized or deenergized parts. This program applies to all work
                operations at <strong>{{ tenant('name') }}</strong> where employees may be exposed to live
                parts and/or those parts
                that have been deenergized.</p>
            <p><strong>{{ $osha->qualified_individual_name }}</strong> has overall responsibility for coordinating
                safety and health programs
                in
                this company. <strong>{{ $osha->service_manager_name }}</strong> is the person having overall
            ...
            <p>Employees of our company who are qualified to work on, near, or with energized electric circuits
                ...
                of such training, has demonstrated an ability to perform duties safely at his or her level of
            ...
            <p>Every employee at <strong>{{ tenant('name') }}</strong> who faces the risk of electric shock
            ...
            <p><strong>{{ tenant('name') }}</strong> conducts the electrical safety training for all
            ...
            <p>It is a <strong>{{ tenant('name') }}</strong> policy that circuits and equipment must be
            ...
            <ul>
                <li><strong>Application of locks and tags.</strong> Only authorized employees are allowed to
                    ...
                    begins. Our locks prevent unauthorized persons from reenergizing the equipment or circuits
                    and
                    the tags prohibit unauthorized operation of the disconnecting device.
                </li>
                <li><strong>Verification of deenergized condition of circuits and equipment.</strong> Prior to
                    work on the
            ...
            <ul>
                ...
                <li>Battery Smart Module (BSM) which communicates on serial data to identifies a number of
                    situations where high voltage should be disabled or any other number of issues identified
                    ...
                    <ul>
                        ...
                        <li>Orange service plug (Disconnect switch) – for additional protection (interrupts the
                            voltage coming out of battery packs).
                        </li>
                    </ul>
                </li>
                <li>Once the vehicles Key is turned off, the BSM Opens the contacts inside the battery
                    disconnecting the High Voltage source from the actual outlet terminals that provide power to
                    the vehicle.
                </li>
            </ul>
            ...
            <ol>
                ...
                <li>Disconnect cable from 12-volt battery on the negative lead and insure cable cannot re-touch
                    battery terminal.
                </li>
                <li><span class="text-red-500">(Put High Voltage Gloves on)</span>
                    <ul>
                        <li>Disconnect the orange service plug on the side of the HV battery and remove and
                            store it on the workbench as well. This gives an additional level of protection in
                ...
                <li>Wait 10 minutes to ensure capacitors are fully discharged
                    <ul>
                        <li>Take your volt meter and put leads on the positive and negative battery terminals
                ...
                <li><span class="text-red-500">(HV Gloves on)</span>
                    <ul>
                        <li>Remove the Power Inverter electrical cover to give you access to the HV terminals
                            inside.
                        </li>
                    </ul>
                </li>
                <li><span class="text-red-500">(HV Gloves on)</span>
                    <ul>
                        <li>Take the voltmeter and touch each HV terminal and check that it is registering “0”
                            volts.
                        </li>
                    </ul>
                </li>
                <li><span class="text-red-500">(HV Gloves on)</span>
                    <ul>
                        <li>Reverse the polarity and check HV terminals again ensuring a reading of “0” volts.
                        </li>
                ...
                <li>Re-check volt meter on the battery terminals again to ensure the meter is still operating
                    properly and that all electrical tests were being recorded accurately at ZERO voltage. Doing
                    this last check is called the “Live Dead Live” procedure.
                </li>
            </ol>
            <h3>Live Dead Live Procedure Completed</h3>
            <p><i>***Now you will know positively that the Hybrid/EV vehicle is not carrying any voltage that
        ...
        <article id="swpp">
            ...
            <p>The purpose of this program is to inform interested persons, including employees, that our
                company is complying with EPA requirements for preparing and maintaining a Stormwater Written
                Pollution Prevention Plan under the stormwater regulations (40 CFR 122).</p>
            <h2>Administrative Duties</h2>
            <p><strong>{{ $osha->service_manager_name }}</strong>, is responsible for our written stormwater pollution
                prevention plan.
                Copies of our written plan may be obtained from service departments office. Employees can obtain
                access to the plan by accessing the supervisor’s office area to review.
                <strong>{{ $osha->service_manager_name }}</strong> and
                <strong>{{ $osha->qualified_individual_name }}</strong>, make up our company's stormwater
            ...
            <p><strong>{{ tenant('name') }}</strong> has no significant materials that have been treated,
            ...
            <p>At our facility, the following activities have a high potential for contaminating stormwater:
                working with the removal and storage of gasoline, oils, antifreeze etc. the following pollutants
            ...
            <p>Our facility has the following materials loading and access areas: parts department loading dock
                area</p>
            <p>To help reduce pollutants in stormwater runoff, we use the following existing structural and
                non-structural control measures: utilizing built in traps and have them cleaned out and removed
                by authorized Haz-Mat companies and to include regularly scheduled actions such as sweeping or
                inspections. </p>
            <p>These measures are located throughout the dealership.</p>
            <p>Our facility's stormwater does receive treatment. The following process is used: local city and
                or county water treatment centers</p>
            <p>The following are predictions of the direction of flow for the types of pollutants likely to be
                present for each area of the plant generating stormwater discharges associated with industrial
                activity: N/A </p>
            ...
            <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for conducting periodic site
                evaluations.</p>
            <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for keeping records of all inspections
                and reports.</p>
            <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for updating the plan as needed.</p>
            <p>The following are the procedures for updating our plan: review annually with department
                supervisor </p>
            <h2>Special Requirements</h2>
            <p><strong>{{ $osha->qualified_individual_name }}</strong> and or
                <strong>{{ $osha->service_manager_name }}</strong> is responsible for incorporating
            ...
            <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for determining if the facility is
                subject to any
                special requirements.</p>
            <p><strong>{{ $osha->service_manager_name }}</strong> is responsible for incorporating into the plan any
                applicable special
                requirements.</p>
            <h2>Appendix</h2>
        ...
        <article id="uomp">
            ...
            <p>Used oil, even when not classified as a hazardous waste under RCRA, can have harmful effects if
                it is released into the environment. In addition, people's health can be affected if used oil is
                handled improperly.</p>
            <p>Superfund regulations allow the federal government to hold any party that created or contributed
                to the creation of a hazardous waste site (including some used oil) financially responsible for
                cleanup costs.</p>
            <p>Used oil is a valuable resource because it has lubrication value and heat value. When treated to
                remove contaminants, the used oil can be used as a base stock to produce new lubricating oil.
                Because used oil has heat value, it can be burned as fuel. Properly burning the used oil keeps
            ...
            <p>This plan provides one source of written documentation for used oil records for
                <strong>{{ tenant('name') }}</strong>. In addition, this plan will inform interested
            ...
            <p>This plan provides a written description of used oil management procedures, disposal methods, and
                transportation requirements. We encourage any suggestions that our employees have for improving
                our written plan for used oil management, as we are committed to developing and maintaining an
            ...
            <h2>Administrative Duties</h2>
            <p>Service manager is responsible for developing the written used oil management plan; for ensuring
                that our written plan is complete, kept up to date, and made available to applicable or required
                authorities; and for maintaining used oil management records. A copy of our used oil management
                plan may be reviewed by employees. It is located in the service manager’s office.</p>
            <h2>Used Oil Defined</h2>
            <p>The EPA defines used oil as "any oil that has been refined from crude oil or any synthetic oil
                ...
                sources, or during industrial operations. This includes oils that are used as hydraulic fluid as
                well as oils that are used to lubricate automobiles and other machinery, cool engines, or
                suspend materials in industrial processes. Oils used for these purposes can become contaminated
            ...
            <p>At this facility, we generate used oil from removal of oil from automobiles for oil change
                operations. It is likely to contain the following contaminants: metals and other engine related
                material</p>
            ...
            <ul>
                ...
                <li>Maintain our collection containers regularly, comply with local fire and safety regulations,
                ...
                <li>Clean up any used oil spills or leaks. This includes providing soak-up material (e.g.,
                    sawdust, kitty litter, or a commercial product) for minor spills. It keeps the area clean
            ...
            <ul>
                ...
                <li>Have constructed secondary containment around our drums/tanks with a capacity for 100
                    percent of the contents of the drums we store; the base of the containment area is sloped so
                    that any spilled oil may be recollected and removed.
                </li>
                <li>Equip storage containers with wide-mouth, long-necked funnels to reduce spills during QUg.
                </li>
                <li>Equip storage containers with a pressure relief valve to reduce a buildup of pressure, which
                    could cause leaks.
                </li>
            ...
            <ul>
                <li>Recycling used oil cashes in on either its lubricating value or heat value. We use this
                    method of management whenever possible because it is easier to do and more cost effective
                    than properly disposing of used oil.
                </li>
                <li>At this facility, we recycle our used oil by having it hauled away by a state approved used
                    oil company method because we do not recycle it or use it for any other function on-site.
                </li>
            </ul>
            ...
            <ul>
                <li>Even though all steps have been taken to prevent leaks or spills from occurring, this
                    company is also prepared to respond to spills of used oil. We instruct workers to use the
                ...
                <li> Stop the release. This action will vary depending on why the release is occurring. For
                    example, if the spill occurs because a 55-gallon drum has been knocked over, the drum should
                    be righted to stop more used oil from being released.
                </li>
                <li>If the spill occurs because a valve on a storage device has been left open, the valve should
                ...
                <li>Contain the release. We strive to prevent the used oil that has been released from
                    spreading. For example, a sorbent, such as kitty litter or sawdust, should be spread over
                    the spilled used oil.
                </li>
                <li>Clean up the release. Depending on the extent of the release, cleaning up the used oil can
                    be a simple or a complicated task. For small spills on the ground, the soil can be dug up
                    ...
                    Because releases that contaminate a great deal of soil or ground or surface water are very
                    difficult to clean up, this company contacts professionals to conduct the cleanup operation.
                </li>
                ...
                <li>Properly manage the solid materials generated during the cleanup. We place solid materials
                    used to clean up a spill of used oil in a sieve-like container to allow the used oil to drip
                    from the solid materials into a storage device. In addition, we compact the materials to
                    remove the used oil. (Removal is complete when there are no more signs of free-flowing oil.)
                    Materials contaminated with used oil that are burned can be managed in the same manner as
                    used oil.
                </li>
                <li>Contaminated materials that will not be burned for energy are tested to determine if they
                    exhibit hazardous waste characteristics. If they do not test hazardous, they are disposed of
            ...
            <ul>
                <li>Although recycling is the best management option for used oil, this company has discovered
                ...
                <li>Because our used oil is contaminated with halogens in concentrations less than 1,000 ppm or
            ...
            <ul>
                <li>Whenever this company changes the oil in a fleet vehicle, the oil filter is also changed to
                    keep the solid contaminants of the old oil from immediately contaminating the new oil. Used
                    oil filters can contain 10 to 16 ounces of used oil; therefore, proper management of this
                    source of used oil is a concern of this company. Used oil filters are not considered a
                    hazardous waste under RCRA if they are not terne-plated and have been properly drained of
                    oil.
                </li>
                <li>When used oil filters are removed from a warm engine, this company uses the Gravity draining
                    - when the filter is removed from the engine, it should be placed with its gasket side down
                    in a drain pan. If the filter has an anti-drain valve, the "dome end" of the filter should
                ...
                <li>We store our drained used oil filters in a covered, rainproof container to prevent used oil
                    from being washed from the filters to the surrounding environment. Our used oil filters are
                    then recycled or properly disposed of.
                </li>
            </ul>
            <p><i>Shipping/Transporting Used Oil</i></p>
            <ul>
                <li>The used oil management standards define a used oil transporter as any person who transports
                    used oil, any person who collects used oil from more than one generator and transports the
                    collected oil, and owners and operators of used oil transfer facilities."
                    <strong>{{ tenant('name') }}</strong> utilizes a waste transport company for hauling our
                    used oil and other products for proper disposal. Our transporter has an EPA ID number and
                    complies with all relevant used oil regulations, including keeping tracking records of where
            ...
            <ul>
                ...
                <li>Measure the level of oil in a tank before and after the hauler collects it to be certain the
                ...
                <li>Ask for a receipt from the transporter that states how much used oil was collected from our
                ...
                <li>Make sure that the hauler maintains storage tanks/containers; labels containers "Used Oil";
                    stores used oil over oil-impervious surfaces; has secondary containment structures in place;
            ...
            <p>Although training is not strictly required under the regulations, we have designated a service
                manager to train personnel who will handle used oil. Direct any questions concerning used oil
            ...
            <p>We keep records of job titles and written job descriptions for all positions related to used oil
                management and the names of employees QUg each job. We also keep records describing the type and
                amount of training provided.</p>
            <h2>Related Requirements</h2>
            <p>Related regulations that we must still comply with include:</p>
            <ul>
                <li>Spill Prevention Control and Countermeasures requirements (40 CFR 112) Please see related
        ...
        <article id="rpp">
            ...
            <h2>Purpose</h2>
            <p>This respirator program lays out standard operating procedures to ensure the protection of all
                employees from respiratory hazards in accordance with the requirements of OSHA 29 CFR
                1910.134.</p>
            <h2>Administrative Duties</h2>
            <p>At <strong>{{ tenant('name') }}</strong> our Respiratory Protection Program Administrator is
                body <strong>{{ $osha->body_shop_manager_name }}</strong>.</p>
            ...
            <p>Employees may review a copy of our Respiratory Protection Program. It is located in the
                supervisor's office, <strong>{{ $osha->body_shop_manager_name }}</strong>'s. Our Program Administrator,
                <strong>{{ $osha->qualified_individual_name }}</strong>, reviews this program periodically to ensure its
                effectiveness.</p>
            <h2>Respirator Selection</h2>
            <p>Respirators are selected on the basis of respiratory hazards to which the worker is exposed and
                workplace and user factors that affect respirator performance and reliability. All selections
                are made by the body shop manager.</p>
            <p>The body shop manager will develop detailed written standard operating procedures governing the
                selection of respirators using the following guidelines: Select and provide respirators based on
                respiratory hazard(s) to which a worker is exposed and workplace and user factors that affect
            ...
            <p>* Provide respirators NIOSH-certified for escape from the atmosphere in which they will be used
                when they are used only for escape from IDLH atmospheres.</p>
            <p>* Consider all oxygen-deficient atmospheres to be IDLH. Exception: If you can demonstrate that,
                under all foreseeable conditions, the oxygen concentration can be maintained within the ranges
                specified in Table II of 29 CFR 1910.134 (i.e., for the altitudes set out in the table), then
            ...
            <ul>
                ...
                <li>An air-purifying respirator, provided that: (1) The respirator is equipped with an
                    end-of-service-life indicator (ESLI) certified by NIOSH for the contaminant; or (2) If there
                    ...
                    that canisters and cartridges are changed before the end of their service life. Describe in
                    the respirator program the information and data relied upon and the basis for the canister
            ...
            <ul>
                <li>An atmosphere-supplying respirator; or</li>
                <li>An air-purifying respirator equipped with a filter certified by NIOSH under 30 CFR part 11
                ...
                <li>For contaminants consisting primarily of particles with mass median aerodynamic diameters
            ...
            <p>For information regarding the types of respirators in use at this facility and their uses, see
                the table in the full plan.</p>
            <h2>Medical Evaluations</h2>
            <p>At <strong>{{ tenant('name') }}</strong>, persons will not be assigned to tasks requiring use
            ...
            <p>Employees must pass one of the fit test types that follow the protocols and procedures contained
                in 29 CFR 1910.134 Appendix A.</p>
            <p>Our workplace-specific fit testing procedures include the following:</p>
            <ol>
                <li>The test subject shall be allowed to pick the most acceptable respirator from a sufficient
                    number of respirator models and sizes so that the respirator is acceptable to, and correctly
                    fits, the user.
                </li>
                <li>Prior to the selection process, the test subject shall be shown how to put on a respirator,
                ...
                <li>The test subject shall be informed that he/she is being asked to select the respirator that
                ...
                <li>The more acceptable facepieces are noted in case the one selected proves unacceptable; the
                    most comfortable mask is donned and worn at least five minutes to assess comfort. Assistance
                    in assessing comfort can be given by discussing the points in the following item A.6. If the
                    test subject is not familiar with using a particular respirator, the test subject shall be
                    directed to don the mask several times and to adjust the straps each time to become adept at
                    setting proper tension on the straps.
                </li>
                <li>Assessment of comfort shall include a review of the following points with the test subject
                ...
                <li>The test subject shall conduct a user seal check, either the negative and positive pressure
                    ...
                    conducting the negative and positive pressure checks, the subject shall be told to seat the
                    mask on the face by moving the head from side-to-side and up and down slowly while taking in
                ...
                <li>Exercise regimen. Prior to the commencement of the fit test, the test subject shall be given
                ...
                <li>Test Exercises. (a) The following test exercises are to be performed for all fit testing
                    ...
                    exercise regimen is contained in the CNP protocol. The test subject shall perform exercises,
                    in the test environment, in the following manner:
                    <ul>
                        <li>(1) Normal breathing. In a normal standing position, without talking, the subject
                            shall breathe normally.
                        </li>
                        <li>(2) Deep breathing. In a normal standing position, the subject shall breathe slowly
                        ...
                        <li>(3) Turning head side to side. Standing in place, the subject shall slowly turn
                            his/her head from side to side between the extreme positions on each side. The head
                            shall be held at each extreme momentarily so the subject can inhale at each side.
                        </li>
                        <li>(4) Moving your head up and down. Standing in place, the subject shall slowly move
                        ...
                        <li>(5) Talking. The subject shall talk out loud slowly and loud enough so as to be
                            heard clearly by the test conductor. The subject can read from a prepared text such
                            as the Rainbow Passage, count backward from 100, or recite a memorized poem or song.
                            ...
                            horizon. There is,
                            according to legend, a boiling pot of gold at one end. People look, but no one ever
                            finds it.
                            When a man looks for something beyond reach, his friends say he is looking for the
                            pot of gold
                            at the end of the rainbow.
                        </li>
                        <li>(6) Grimace. The test subject shall grimace by smiling or frowning. (This applies
                        ...
                        <li>(b) Each test exercise shall be performed for one minute except for the grimace
                            ...
                            become
                            unacceptable, another model of respirator shall be tried. The respirator shall not
                            be adjusted
                            once the fit test exercises begin. Any adjustment voids the test, and the fit test
                            must be
                            repeated. B.
                        </li>
                    </ul>
            ...
            <p>* For all tight-fitting respirators, ensure that employees perform a user seal check each time
            ...
            <ul>
                <li>To wash their faces and respirator facepieces as necessary to prevent eye or skin irritation
                    associated with respirator use; or - If they detect vapor or gas breakthrough, changes in
                ...
                <li>If the employee detects vapor or gas breakthrough, changes in breathing resistance, or
                    leakage of the facepiece, replace or repair the respirator before allowing the employee to
            ...
            <p>* The employer or designee is notified before the employee(s) located outside the IDLH atmosphere
            ...
            <ul>
                ...
                <li>Equivalent means for rescue where retrieval equipment is not required under the bullet item
                    above this one.
                </li>
            </ul>
            <p>Procedures for Interior Structural Firefighting</p>
            <p>In addition to the requirements set forth in the row above for Procedures for IDLH Atmospheres,
                in interior structural fires, ensure that:</p>
            <p>* At least two employees enter the IDLH atmosphere and remain in visual or voice contact with one
            ...
            <p>Our company provides each respirator user with a respirator that is clean, sanitary, and in good
            ...
            <p>* Respirators issued to more than one employee must be cleaned and disinfected before being worn
            ...
            <p>We ensure that respirators are stored according to 29 CFR 1910.134(h)(2). They are packed or
                stored in employees’ lockers to prevent deformation of the facepiece and exhalation valve.</p>
            <p>We inspect respirators according to the requirements in 29 CFR 1910.134(h)(3). We have created
                the following inspection schedule(s) to be used for each respirator: All types of respirators
                used in routine situations must be inspected before each use and during cleaning.</p>
            <p>* Respirators maintained for use in emergency situations must be inspected at least monthly and
                in accordance with the manufacturer's recommendations, and checked for proper function before
            ...
            <p>The following detailed procedures ensure adequate air quality, quantity, and flow of breathing
            ...
            <h2>Training</h2>
            <p>Employee training is an important part of the respiratory protection program and is essential for
            ...
            <p>At <strong>{{ tenant('name') }}</strong>, program evaluation is performed annually by our
        ...
        <article id="bpp">
            ...
            <p>Our exposure control plan (ECP) is provided to eliminate or minimize occupational exposure to
                bloodborne pathogens in accordance with OSHA standard 29 CFR 1910.1030, "Occupational Exposure
                to Bloodborne Pathogens."</p>
            <h2>Administrative Duties</h2>
            <p><strong>{{ $osha->qualified_individual_name }}</strong>, is responsible for the implementation of the ECP
                and is responsible
                ...
                responsible for ensuring that all medical actions are performed and for maintaining appropriate
                records. <strong>{{ $osha->qualified_individual_name }}</strong> is responsible for training and
                allowing ECP access for
            ...
            <p>Covered employees receive an explanation of this ECP during their initial training, and annually.
                ...
                <strong>{{ $osha->qualified_individual_name }}</strong>. The plan indicates which devices have been
                identified as candidates
                for our use. <strong>{{ $osha->qualified_individual_name }}</strong> solicits input from non-managerial
                employees responsible
                for direct patient care in the identification, evaluation, and selection of effective
                engineering and work practice controls. <strong>{{ $osha->qualified_individual_name }}</strong>
                documents all solicitation in
                the ECP and indicates the engineering and work practice controls identified during solicitation
                in our annual reviews.</p>
            <p><i>Engineering and Work Practice Controls</i></p>
            <p>The plan indicates which specific engineering controls and work practice controls are used. The
                controls include how sharps disposal containers are maintained. The plan also indicates how we
                identify the need for changes in engineering control and work practices, and evaluates the need
                for new procedures or new products. <strong>{{ $osha->qualified_individual_name }}</strong> is
                responsible for the effective
                implementation controls recommendations.</p>
            <p><i>Personal Protective Equipment (PPE)</i></p>
            <p>PPE is provided to our employees at no cost to them.
                <strong>{{ $osha->qualified_individual_name }}</strong> provides
            ...
            <p>The plan indicates which labeling method(s) is used in this facility.
                <strong>{{ $osha->service_manager_name }}</strong>,
                ensures that proper, effective labeling is used. The local health department and
                <strong>{{ tenant('name') }}</strong> provides training to employees on hepatitis B
            ...
            <p>Should an exposure incident occur, employees are to contact
                <strong>{{ $osha->qualified_individual_name }}</strong>. We
                provide immediate, confidential medical evaluation and follow-up after initial first aid.</p>
            <p><i>Administration of Post-Exposure Evaluation and Follow-up</i></p>
            <p><strong>{{ $osha->qualified_individual_name }}</strong> ensures that the responsible health care
                professional(s) is given a
                copy of OSHA's bloodborne pathogens standard, and
                <strong>{{ $osha->qualified_individual_name }}</strong> provides the
            ...
            <p><strong>{{ $osha->qualified_individual_name }}</strong> reviews the circumstances of all exposure
                incidents. If necessary,
                <strong>{{ $osha->qualified_individual_name }}</strong> ensures that appropriate revisions are made to
                this ECP.</p>
            <h2>Employee Training</h2>
            <p><strong>{{ $osha->qualified_individual_name }}</strong> conducts training for employees who have
            ...
            <p>Training records are kept for at least three years at
                <strong>{{ $osha->qualified_individual_name }}</strong> office location.
                The plan describes which training records are included.</p>
            <p><i>Medical Records</i></p>
            <p><strong>{{ $osha->qualified_individual_name }}</strong> is responsible for maintaining the required
                medical records. These
                records are kept in the human resource office for at least the duration of employment plus 30
                years.</p>
            <p><i>OSHA Recordkeeping</i></p>
            <p>If an exposure incident occurs, it is evaluated, and
                <strong>{{ $osha->qualified_individual_name }}</strong> determines if the
        ...
        <article id="ppe">
            ...
            <p>While OSHA's Personal Protective Equipment regulation, found at 29 CFR 1910.132-.140 (Subpart I),
                does not explicitly require a written Personal Protective Equipment (PPE) Program,
                <strong>{{ tenant('name') }}</strong> has developed a written PPE program to document and
                specify all information relative to our PPE needs.</p>
            <p>The <strong>{{ $osha->service_manager_name }}</strong>, is the program coordinator, acting as the
                ...
                their use of PPE. This written plan is kept in service department office will review and update
            ...
            <p>During the walk-through survey the Safety Manager observes and records the hazards along with PPE
                currently in use (type and purpose). The Safety Manager organizes the data and information for
            ...
            <p>Once any hazards have been identified and evaluated through hazard assessment, protective
                equipment is chosen according to the selection guidelines in Appendix B to Subpart I of 29 CFR
                1910.</p>
            <p>It is the responsibility of the Safety Manager to reassess the workplace hazard situation as
                necessary, to identify and evaluate new equipment and processes, to review accident records, and
                reevaluate the suitability of previously selected PPE. This reassessment will take place as
                needed, but at least every quarter during inspection of the facility and work environment.</p>
            <h2>Employee Training</h2>
            ...
            <p>Because failure to comply with company policy concerning PPE can result in OSHA citations and
                ...
                disciplined for noncompliance according to the following schedule: employee will be subject to
            ...
            <p><span class="italic underline">Safety Shields / Goggles</span> - Work Area Hazard: grinding,
                welding flash, chemical splash, high temperature, splash from molten metal’s, hammering, metal
                fragments.</p>
            <p>Type of goggles/face shield: covered goggles, direct ventilation, face shields, welding
                helmets.</p>
            <p>Employees from temporary work agencies and contractors are required to wear goggles/face shields
            ...
            <p>Type of safety shoe: Rubber heeled boots with Nitrile or Neoprene boots soles to protect against
            ...
            <p>Members of the Emergency Response Team are required to wear safety footwear when responding to
            ...
            <p>All employees who work in designated work areas and/or job assignments are responsible for
                ...
                Employees needing to purchase their own safety shoes through a local merchant as a condition of
                employment.</p>
            <h2>PPE Specific Information</h2>
            ...
            <p>Type of glove: gloves will be used and assessed based on the type of job function being employed.
            ...
            <p>Work area: service technicians and parts department employees. </p>
            <p>Hazard: hair from oil, grease, water, dirt and sparks protect the head from hot exhaust systems,
                hot oil, sharp protruding objects and falling pipes, mufflers or other vehicle parts. They
                easily adapt for the wearing of eye goggles and safety glasses. Parts department employees can
            ...
            <p>Employees from temporary work agencies and contractors are required to wear hard hats if assigned
                to work in the designated work areas.</p>
        </article>
        <article id="cgp">
            <h1 class="text-arm-blue-600">Compressed Gas Plan</h1>
            <h2>Purpose</h2>
            <p>It is the policy of <strong>{{ tenant('name') }}</strong> to permit only trained and
            ...
            <p><span class="italic underline">Handling Procedures</span>- We follow the safe handling procedures
                found in the CGA pamphlet
                series, including the P-1-1991 pamphlet.</p>
            <p><span class="italic underline">Storage Procedures</span> - We follow the safe storage procedures
                found in the CGA pamphlet series,
                including the P-1-1991 pamphlet.</p>
            <p><span class="italic underline">Usage Procedures</span> - We follow the safe storage procedures
                found in the CGA pamphlet series,
                including the P-1-1991 pamphlet.</p>
            <h2>Training Program</h2>
        ...
        <article id="wcp">
            ...
            <h2>Maintenance</h2>
            <p><strong>{{ $osha->parts_manager_name }}</strong>, <strong>Parts Manager</strong> complete(s) a receiving
                or delivery
            ...
            <p>(c)(1)(iv) Precautionary labels. A number of potentially hazardous materials are employed in
                ...
                cutting,
                etc.</p>
            <p>(c)(1)(iv)(A) All filler metals and fusible granular materials shall carry the following notice,
                as a
            ...
            <h2>Recordkeeping</h2>
            <p><strong>{{ $osha->service_manager_name }}</strong>, <strong>Service Manager</strong> is responsible for
        ...
        <article id="fpp">
            ...
            <ul>
                ...
                <li>The type of fire protection equipment or systems which can control a fire involving them.
                </li>
            ...
            <p><strong>{{ $osha->qualified_individual_name }}</strong>, is the Plan Coordinator, acting as the
                representative of the Facility
                Manager, who has overall responsibility for the plan. The written plan is kept in Qualified
                Individual’s office and or online compliance dashboard.
                <strong>{{ $osha->qualified_individual_name }}</strong>, will review
                and
            ...
            <p>The FPP communicates to employees, policies and procedures to follow when fires erupt. This
                ...
                OSHA officials who ask to see it.</p>
            <p>If after reading this plan, you find that improvements can be made, please contact the Plan
                Coordinator, <strong>{{ $osha->qualified_individual_name }}</strong>. We encourage all suggestions
                because we are committed
                to the success of our Fire Prevention Plan. We strive for clear understanding, safe behavior,
                and
                involvement in the plan from every level of the company.</p>
            <h2>Plan Coordinator Responsibilities</h2>
            <p>Here at <strong>{{ tenant('name') }}</strong>, the Plan Coordinator,
                <strong>{{ $osha->qualified_individual_name }}</strong>, or designee, is responsible
            ...
            <ol>
                ...
                <li>If evacuation is deemed necessary, the Plan Coordinator ensures that:
                    <ul>
                        ...
                        <li>The building owner/superintendent is contacted, informed of the action taken, and
                            asked
            ...
            <p>In addition, the Plan Coordinator is responsible for duties unique to this facility. Storage of
                used
            ...
            <p>Fire can be represented by a simple equation: Fire = Ignition Source + Fuel + Oxygen. Without
                any one of these three elements, a fire cannot start. Likewise, during a fire, if you take away
                any
                one of these three elements, you can successfully put out a fire. It is our company&#39;s intent
                to
            ...
            <p>The service and parts manager along with, <strong>{{ $osha->qualified_individual_name }}</strong>, will
            ...
            <p>Fire protection equipment, selected and purchased by service manager, in use at this
            ...
            <ul>
                ...
                <li>Class C—This is an electrical fire. Carbon dioxide or dry chemicals extinguish this fire.
                </li>
            ...
            <p>Once hazards are evaluated and equipment is installed to control them, the equipment must be
                ...
                fires: <strong>{{ $osha->service_manager_name }}</strong>.</p>
            ...
            <ul>
                ...
                <li>Measures to contain fire (e.g., closing office doors, windows, etc. in immediate vicinity)
                </li>
        ...
        <article id="mesg">
            ...
            <p>This written Machine/Equipment Safety and Guarding Plan describes methods and practices for
                ...
                managers, supervisors, and employees at {{ tenant('name') }}. This written plan is intended to be used
                to:</p>
            <ul>
                ...
                <li>Demonstrate <strong>{{ tenant('name') }}&#39;s</strong> compliance with machine safety
                    and equipment usage
                    requirements for general industry in Subpart O and P of 29 CFR 1910.
                </li>
            </ul>
            <h2>Administrative Duties</h2>
            <p><strong>{{ $osha->service_manager_name }}</strong>, our company&#39;s <strong>Service Manager</strong>,
                is responsible for
            ...
            <p><strong>{{ $osha->qualified_individual_name }}</strong> will identify all new employees in the employee
                Orientation Program
                and
            ...
            <p><strong>{{ $osha->service_manager_name }}</strong>, and assigned supervisors are accountable for all
                phases of
                installation and
            ...
            <p>Although we may not be able to eliminate all problems, we try to eliminate as many as possible
                ...
                <strong>{{ $osha->service_manager_name }}</strong>, is responsible for evaluating and updating this
                written plan. The
                evaluation will
