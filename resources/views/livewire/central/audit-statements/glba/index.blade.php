    ...
    <div>
        <div class="sm:flex sm:items-center">
            ...
            <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none flex gap-3">
                <x-button size="sm" target="_blank" href="{{ route('glba-violations.print') }}">Print View</x-button>
                @role('super-admin')
                    <x-button href="{{ route('glba-violations.create') }}" variant="primary" size="sm">Add Violation</x-button>
                @endrole
            </div>
        </div>
                            ...
                            @foreach($violations as $violation)
                                <x-table.row>
                                    ...
                                    <x-table.cell class="text-right">
                                        <x-button href="{{ route('glba-violations.edit', $violation) }}" size="xs" variant="ghost">Edit</x-button>
                                    </x-table.cell>
                                </x-table.row>
                            @endforeach
                        </x-slot:body>
