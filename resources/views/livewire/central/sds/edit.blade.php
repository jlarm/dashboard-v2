<div class="p-6">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-xl font-bold leading-none tracking-tight text-neutral-900">Edit {{ $sds->name }}</h1>
        </div>
    </div>
    <form wire:submit.prevent="update" class="max-w-4xl mx-auto space-y-5 mt-8" @keydown.enter.prevent>
        ...
        <div class="flex items-center justify-between p-3 border rounded-lg">
            <div class="flex items-center gap-1">
                <svg class="flex-shrink-0 size-5" width="400" height="492" viewBox="0 0 400 492" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span class="font-mono text-xs text-gray-500">{{ $file }}</span>
            </div>
            <button type="button" wire:click="deleteFile" class="inline-flex flex-shrink-0 justify-center items-center ms-1 rounded-md px-2 py-1 text-white bg-red-500 hover:bg-red-700 focus:outline-none focus:bg-red-700 text-xs">
                Remove
            </button>
        </div>
        @endif
        ...
        <div class="flex justify-start items-baseline gap-3 border border-gray-300 rounded-md p-3">
            <label for="file" class="hover:cursor-pointer">
                <div class="py-1 px-2 inline-flex items-center gap-x-2 text-xs font-medium rounded-md border bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none">
                    Select PDF
                </div>
                <input class="sr-only" id="file" type="file" wire:model="file" />
            </label>
            @if($file)
                <p class="block text-xs text-gray-500 mt-2">{{ $file->temporaryUrl() }}</p>
            @endif
        </div>
        @endif
        <x-primary-button wire:loading.attr="disabled" wire:loading.class="opacity-25" wire:target="update">
            Update
            <svg wire:loading wire:target="update" class="animate-spin ml-1 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg"
                 fill="none"
                 viewBox="0 0 24 24">
