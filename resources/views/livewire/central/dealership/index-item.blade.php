<div class="group relative overflow-hidden flex flex-col justify-between rounded-lg border border-gray-200 bg-white shadow-sm transition-all hover:shadow-md">
    <div class="p-6">
        <div class="flex items-start flex-col justify-between gap-3">
            <h3 class="font-medium text-gray-900 min-w-0 flex-1">{{ $dealership->name }}</h3>
            @role('super-admin')
                <div
                    x-data="{
    ...
    <div class="border-t border-gray-100 bg-gray-50 p-3">
        ...
        @can('delete-dealership')
            <button
                type="button"
                x-on:click="if (confirm('Are you sure you want to delete {{ $dealership->name }}? The dealership database will be preserved and this can be reversed.')) { $wire.deleteDealership() }"
