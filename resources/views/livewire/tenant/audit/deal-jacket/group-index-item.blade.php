<x-table.row>
            ...
            @if(!$dealJacketGroup->completed)
                <div>
                  <span class="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-lg">
                    <svg class="shrink-0 size-3 animate-spin" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <x-table.cell class="w-16 flex justify-end !pr-0">
        <x-menu-dropdown align="right" width="48">
            ...
            <x-slot:content>
                <x-menu-dropdown-item href="{{ tenant('locations')
                    ? route('dealer.stores.audits.deal-jackets.show', [$store, $dealJacketGroup])
                    : route('dealer.audit.deal-jackets.show', $dealJacketGroup->uuid) }}">
