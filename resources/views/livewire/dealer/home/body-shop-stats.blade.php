<div>
    <a href="{{ !tenant('locations') ? route('dealer.audit.body-shop.index') : route('dealer.stores.audits.body-shop.index', $store) }}"
       class="flex flex-col gap-y-4 bg-white rounded border hover:shadow-xl transition pt-10">
        <dt class="text-base leading-7 text-gray-600">Body Shop Rating</dt>
        @if($rating > 0)
            @if($rating >= 90)
    ...
    @if(count($audits) > 1)
        <script>
            ...
            var chart = new ApexCharts(document.querySelector("#body-shop-chart"), options);
