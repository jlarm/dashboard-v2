<div>
    <div class="relative overflow-hidden p-4 sm:p-5 bg-white border border-gray-200 rounded-xl shadow-sm before:absolute before:top-0 before:end-0 before:size-full before:bg-gradient-to-br before:from-purple-100 before:via-transparent before:blur-xl">
        <a href="{{ !tenant('locations') ? route('dealer.audit.body-shop.index') : route('dealer.stores.audits.body-shop.index', $store) }}" class="relative z-0">
            <!-- Header -->
            <div class="flex justify-between items-center gap-x-3">
                ...
                <span class="mb-3 inline-flex justify-center items-center size-8 md:size-10 rounded-lg bg-white text-gray-700 shadow dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-400">
                    <svg class="flex-shrink-0 size-4 md:size-5 text-purple-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none">
                ...
                @if($this->progress() != null)
                    @if($this->progress()[0] === 'positive')
                        <span class="flex-shrink-0 py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-teal-100 text-teal-800 rounded-full dark:bg-teal-500/10 dark:text-teal-500">
                    ...
                    @else
                        <span class="flex-shrink-0 py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-red-100 text-red-800 rounded-full dark:bg-teal-500/10 dark:text-teal-500">
                          <svg class="w-3 h-3" width="16" height="16" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
