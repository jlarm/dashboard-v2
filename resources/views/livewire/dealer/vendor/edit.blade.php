<x-wire-elements-pro::tailwind.slide-over on-submit="send" :content-padding="false">
    ...
    <div class="flex flex-col h-full">
        <div class="border-t border-gray-200 p-3 flex-1 overflow-y-auto">
            ...
            <div class="space-y-6" x-data="{ openForm: null }">
                @foreach($forms as $form)
                    <div class="relative">
                        <div
                            x-on:click="openForm = openForm === {{ $form->id }} ? null : {{ $form->id }}"
                            class="flex items-start justify-between cursor-pointer group hover:bg-gray-50 -mx-2 px-2 py-2 rounded-lg transition"
                        >
                            ...
                            <div class="flex flex-col items-end">
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    <span class="inline-block px-2 py-0.5 text-[10px] font-bold rounded uppercase {{ $form->signature ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">{{ $form->signature ? 'Completed' : 'Pending' }}</span>
                                </div>
                            </div>
                        </div>
                        <div
