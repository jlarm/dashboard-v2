<x-wire-elements-pro::tailwind.modal>
    <div class="relative z-10" role="dialog" aria-modal="true">
        <div class="mx-auto max-w-xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white ring-1 ring-black ring-opacity-5 transition-all">
            <div class="relative">
                ...
                <ul class="max-h-64 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800" id="options" role="listbox">
                    @if(count($violations) > 0)
                        @foreach($violations as $violation)
                            <li wire:click="selectViolation({{ $violation->id }})"
            ...
            @elseif ($violations === null || count($violations) === 0)
            @else
