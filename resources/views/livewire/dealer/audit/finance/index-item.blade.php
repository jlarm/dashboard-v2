    ...
    <div class="space-y-1">
        <div class="flex justify-between">
            ...
            >
                ...
                >
                    @can('create-audits')
                        <a href="{{ tenant('locations') ? route('dealer.stores.audits.finance.edit', [$store, $glbaViolationAudit->uuid]) : route('dealer.audit.finance.edit', $glbaViolationAudit->uuid) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900">
                    ...
                    @endcan
                        @if($glbaViolationAudit->pdf_path)
                            @can('view-audits')
                                <a href="{{ tenant('locations') ? route('dealer.stores.audits.finance.view', [$store, $glbaViolationAudit->uuid]) : route('dealer.audit.finance.remediation', $glbaViolationAudit->uuid) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" color="#4a4a4a" fill="none">
                            ...
                            @endcan
                        @endif
                    <a href="{{ tenant('locations') && auth()->user()->can('create-stores') ? route('dealer.stores.audits.finance.view', [$store, $glbaViolationAudit->uuid]) : route('dealer.audit.finance.show', $glbaViolationAudit->uuid) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" color="#4a4a4a" fill="none">
                    ...
                    @can('delete-audits')
                        ...
                        <button wire:click="$emit('modal.open', 'dealer.audit.finance.delete',  @js(['glbaViolationAudit' => $glbaViolationAudit->id]))" class="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-500 hover:bg-gray-100 hover:text-red-600">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none">
        ...
        <span class="text-xs text-gray-600">
          Total Remediations:
           ...
           {{ $glbaViolationAudit->grade ? $glbaViolationAudit->remediation_count : '-' }}
        </span>
        </div>
        <!-- End Item -->
        ...
        <span @class([
            ...
            'bg-teal-100 text-teal-800' => $glbaViolationAudit->grade != null,
            'bg-sky-100 text-sky-800' => $glbaViolationAudit->grade === null
        ])
          >
        {{ $glbaViolationAudit->grade ? 'Complete' : 'In Progress' }}
        </span>
        </div>
        <!-- End Item -->
    </div>
