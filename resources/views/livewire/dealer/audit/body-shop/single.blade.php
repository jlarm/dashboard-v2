<div class="mb-10">
    <div class="w-full max-w-3xl mx-auto px-6">
            ...
            <div>
                ...
                <div class="mb-3 flex flex-col md:flex-row justify-between items-center gap-3">
                    <span class="block text-xl font-semibold text-gray-800">
                        Body Shop Audit for {{ $bodyShopViolationAudit->date->format('F d, Y') }}
                    </span>
                    @if($bodyShopViolationAudit->pdf_path)
                        <span class="flex-shrink-0 inline-flex items-center gap-x-1.5 py-0.5 px-1.5 rounded-md text-xs font-medium bg-teal-100 text-teal-800">Completed</span>
                    @else
                        <span class="flex-shrink-0 inline-flex items-center gap-x-1.5 py-0.5 px-1.5 rounded-md text-xs font-medium bg-sky-100 text-sky-800">In Progress</span>
                    @endif
                </div>
                <!-- End Header -->
            </div>
            ...
            @forelse($violations as $violation)
                <div class="space-y-5 border rounded-md p-5 relative">
                    ...
                    @if($violation->violation_date)
                        <p class="text-sm">Violation Date: {{ $violation->violation_date->format('F d, Y') }}</p>
                    @endif
        ...
        <a class="inline-flex items-center px-4 py-2 bg-arm-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-arm-blue-700 focus:bg-arm-blue-700 active:bg-arm-blue-900 focus:outline-none focus:ring-2 focus:ring-arm-blue-500 focus:ring-offset-2 transition ease-in-out duration-150 mt-5" href="{{ url()->previous() }}">Back</a>
    </div>
</div>
