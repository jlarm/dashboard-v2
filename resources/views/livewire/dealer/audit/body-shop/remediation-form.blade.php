            ...
            <div>
                ...
                <div class="mb-3 flex flex-col md:flex-row justify-between items-center gap-3">
                    <span class="block text-xl font-semibold text-gray-800">
                       Remediating the Body Shop Audit for {{ $bodyShopViolationAudit->date->format('F d, Y') }}
                    </span>
                </div>
                <!-- End Header -->
            </div>
            ...
            <div class="ml-auto flex gap-2">
                @can('create-stores')
                    <button
                        ...
                        wire:click="$emit('modal.open', 'dealer.audit.body-shop.complete-remediation-modal',  @js(['bodyShopViolationAudit' => $bodyShopViolationAudit->id]))"
                    >
                        Generate PDF
                    </button>
                @endcan
                <x-button.secondary href="{{ tenant('locations') ? route('dealer.stores.audits.body-shop.index', $store) : route('dealer.audit.body-shop.index') }}">Cancel</x-button.secondary>
            </div>
        </div>
    </div>
</div>
