        ...
        <x-slot name="actions">
            @can('create-audits')
                <a
                    ...
                    href="{{ route('dealer.audit.body-shop.create', $store) }}"
                >
                    Create Audit
                </a>
            @endcan
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @if(!empty($chartLabels) && !empty($chartGradesNumeric))
                <div class="bg-white shadow-sm border rounded-lg p-6">
                    ...
                    <div class="relative h-64">
                        <div id="bodyShopGradeChart-{{ $this->id }}"></div>
                    </div>
                </div>
            @endif
            ...
            @if(!empty($chartLabels) && !empty($chartViolations))
                <div class="bg-white shadow-sm border rounded-lg p-6">
                    ...
                    <div class="relative h-64">
                        <div id="bodyShopViolationsChart-{{ $this->id }}"></div>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="w-full bg-white">
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-4">
                @foreach($audits as $bodyShopAudit)
                    <livewire:dealer.audit.body-shop.index-item :store="$store" :bodyShopAudit="$bodyShopAudit" :key="$bodyShopAudit->id"/>
                @endforeach
                @foreach($bodyShopAudits as $audit)
                    <livewire:dealer.audit.body-shop.old-audit-index :bodyShopAudit="$audit" :key="$audit->id"/>
                @endforeach
                @if(!$audits->count() && !$bodyShopAudits->count())
                    <!-- Empty State -->
...
<script>
    (function() {
        ...
        const chartElementId = 'bodyShopGradeChart-' + componentId;
    ...
    (function() {
        ...
        const componentId = '{{ $this->id }}';
        const chartElementId = 'bodyShopViolationsChart-' + componentId;
