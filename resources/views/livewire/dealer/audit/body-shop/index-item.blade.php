    ...
    <div class="p-4">
                        ...
                        <div class="space-y-4 p-6">
                            ...
                            <div class="space-y-3 rounded-lg border border-gray-200 p-4">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">{{ $this->bodyShopAudit->remediation_count }} of {{ $this->bodyShopAudit->violation_count }} remediated</p>
                                </div>
                                @endif
                                <div class="grid grid-cols-2 gap-x-4 gap-y-2 pt-2 text-sm">
                                    <div>
                                        <dt class="text-gray-500">Grade:</dt>
                                        <dd class="font-medium text-gray-800">{{ $bodyShopAudit->grade ?? '-' }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-gray-500">Status:</dt>
                                        <dd @class(['font-medium', 'text-teal-600' => $bodyShopAudit->completed_date, 'text-sky-600' => !$bodyShopAudit->completed_date])>
                                            {{ $bodyShopAudit->completed_date ? 'Complete' : 'In Progress' }}
                                        </dd>
                                    </div>
                                    <div>
                                        <dt class="text-gray-500">Violations:</dt>
                                        <dd class="font-medium text-gray-800">{{ $bodyShopAudit->violation_count }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-gray-500">Remediations:</dt>
                                        <dd class="font-medium text-gray-800">{{ $bodyShopAudit->completed_date ? $bodyShopAudit->remediation_count : '-' }}</dd>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-1 pt-2">
                                ...
                                @can('create-audits')
                                    <a href="{{ tenant('locations') ? route('dealer.stores.audits.body-shop.edit', [$store, $bodyShopAudit->uuid]) : route('dealer.audit.body-shop.edit', $bodyShopAudit->uuid) }}" class="group flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-900">
                                ...
                                @endcan
                                @if($this->remediationsActive() && $bodyShopAudit->completed_date)
                                    @can('view-audits')
                                        <a href="{{ tenant('locations') ? route('dealer.stores.audits.body-shop.remediation', [$store, $bodyShopAudit->uuid]) : route('dealer.audit.body-shop.remediation', $bodyShopAudit->uuid) }}" class="group flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-900">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" class="text-gray-400 group-hover:text-gray-500" fill="none">
                                ...
                                @endif
                                @can('view-audits')
                                    @if($bodyShopAudit->violation_pdf_path)
                                        <div class="px-1 py-1.5"><livewire:dealer.audit.body-shop.generate-button :bodyShopViolationAudit="$bodyShopAudit" /></div>
                                    @endif
                                @endcan
                                @if($bodyShopAudit->pdf_path)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
            ...
            <div>
                ...
                <dd class="mt-0.5 font-semibold text-slate-700 tracking-tight">{{ $bodyShopAudit->grade ?? '-' }}</dd>
            </div>
            ...
            <div>
                ...
                <dd class="mt-0.5">
                    <span @class([
                        ...
                        'bg-emerald-100 text-emerald-700' => $bodyShopAudit->completed_date,
                        'bg-sky-100 text-sky-700' => !$bodyShopAudit->completed_date
                    ])>
                        @if($bodyShopAudit->completed_date)
            ...
            <div>
                ...
                <dd class="mt-0.5 font-semibold text-slate-700 tracking-tight">{{ $bodyShopAudit->violation_count }}</dd>
            </div>
            ...
            <div>
                ...
                <dd class="mt-0.5 font-semibold text-slate-700 tracking-tight">{{ $bodyShopAudit->completed_date ? $bodyShopAudit->remediation_count : '-' }}</dd>
            </div>
            @endif
        </div>
    </div>
