<x-wire-elements-pro::tailwind.modal onSubmit="confirm" :contentPadding="false">
    <div class="space-y-6 my-6">
        <div class="px-4">
            <div class="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
                <button type="button" wire:click="$emit('modal.close')" class="rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
    ...
    <x-slot name="buttons">
        <div class="flex justify-between w-full">
            <button type="button" wire:click="$emit('modal.close')" class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 mb-2" wire:loading.attr="disabled">
                {{ $prompt['cancel'] }}
            </button>

            <button type="submit" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 focus:outline-none" wire:loading.attr="disabled">
                {{ $prompt['confirm'] }}
            </button>
        </div>
    </x-slot>
