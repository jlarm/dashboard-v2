<?php

declare(strict_types=1);

use App\Http\Livewire\Tenant\Audit\DealJacket\CreateNewGroupButton;
use App\Http\Livewire\Tenant\Audit\DealJacket\DealJacketGroupDeleteModal;
use App\Models\Dealer\Audit\DealJacketGroup;
use App\Models\Dealer\Store;
use Livewire\Livewire;

describe('Deal Jacket Group Index Page', function () {
    describe('Page Access', function () {
        it('allows consultants to access the index page with all components', function () {
            $this->actingAs($this->consultant)
                ->get(route('dealer.audit.deal-jackets.index'))
                ->assertStatus(200)
                ->assertSee('Deal Jacket Audits')
                ->assertSeeLivewire('tenant.audit.deal-jacket.pass-rate-trend-chart')
                ->assertSeeLivewire('tenant.audit.deal-jacket.common-issues-chart')
                ->assertSeeLivewire('tenant.audit.deal-jacket.create-new-group-button')
                ->assertSeeLivewire('tenant.audit.deal-jacket.group-index');
        });
    });

    describe('Multi-Store Data Isolation', function () {
        it('consultants only see deal jacket groups belonging to their current store', function () {
            // Get the default test store
            $storeA = Store::first();

            // Create a second store within the same tenant
            $storeB = Store::create([
                'name' => 'Second Store',
                'slug' => 'second-store',
            ]);

            // Create groups for Store A
            DealJacketGroup::factory()->count(2)->create(['store_id' => $storeA->id]);

            // Create groups for Store B
            DealJacketGroup::factory()->count(3)->create(['store_id' => $storeB->id]);

            // Bind Store A as the current store
            app()->instance('currentStore', $storeA->id);

            // Verify Store A only sees their 2 groups
            expect(DealJacketGroup::where('store_id', $storeA->id)->count())->toBe(2);
            expect(DealJacketGroup::where('store_id', $storeB->id)->count())->toBe(3);

            $response = $this->actingAs($this->consultant)
                ->get(route('dealer.audit.deal-jackets.index'));

            $response->assertOk();

            // The Livewire component should only show Store A's groups
            $groups = DealJacketGroup::where('store_id', $storeA->id)->get();
            expect($groups->count())->toBe(2);
        });

        it('managers only see completed deal jacket groups belonging to their current store', function () {
            $storeA = Store::first();

            $storeB = Store::create([
                'name' => 'Second Store',
                'slug' => 'second-store',
            ]);

            // Create completed groups for Store A
            DealJacketGroup::factory()->count(2)->create([
                'store_id' => $storeA->id,
                'completed' => true,
            ]);

            // Create completed groups for Store B
            DealJacketGroup::factory()->count(3)->create([
                'store_id' => $storeB->id,
                'completed' => true,
            ]);

            // Bind Store A as the current store
            app()->instance('currentStore', $storeA->id);

            // Verify data is correctly scoped
            expect(DealJacketGroup::where('store_id', $storeA->id)->where('completed', true)->count())->toBe(2);
            expect(DealJacketGroup::where('store_id', $storeB->id)->where('completed', true)->count())->toBe(3);

            $response = $this->actingAs($this->manager)
                ->get(route('dealer.audit.deal-jackets.index'));

            $response->assertOk();
        });

        it('consultants switching store context see different deal jacket groups', function () {
            $storeA = Store::first();

            $storeB = Store::create([
                'name' => 'Second Store',
                'slug' => 'second-store',
            ]);

            // Create groups for both stores
            DealJacketGroup::factory()->count(2)->create(['store_id' => $storeA->id]);
            DealJacketGroup::factory()->count(3)->create(['store_id' => $storeB->id]);

            // Test as Store A
            app()->instance('currentStore', $storeA->id);

            expect(DealJacketGroup::where('store_id', $storeA->id)->count())->toBe(2);

            $responseA = $this->actingAs($this->consultant)
                ->get(route('dealer.audit.deal-jackets.index'));

            $responseA->assertOk();

            // Switch to Store B
            app()->instance('currentStore', $storeB->id);

            expect(DealJacketGroup::where('store_id', $storeB->id)->count())->toBe(3);

            $responseB = $this->actingAs($this->consultant)
                ->get(route('dealer.audit.deal-jackets.index'));

            $responseB->assertOk();
        });
    });

    describe('Consultant Permissions', function () {
        describe('Viewing Groups', function () {
            it('consultants can see deal jacket group when not completed', function () {
                $dealJacketGroup = DealJacketGroup::factory()->create();

                $this->actingAs($this->consultant)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertSee($dealJacketGroup->created_at->format('M d, Y'))
                    ->assertSee('In Progress');
            });
        });

        describe('Creating Groups', function () {
            it('consultants can create a new deal jacket group', function () {
                $this->actingAs($this->consultant);
                $storeId = Store::first()->id;

                // Bind the current store to the app container (normally done by middleware)
                app()->instance('currentStore', $storeId);

                expect(DealJacketGroup::count())->toBe(0);

                Livewire::test(CreateNewGroupButton::class)
                    ->call('create')
                    ->assertRedirect();

                expect(DealJacketGroup::count())->toBe(1);

                $dealJacketGroup = DealJacketGroup::first();
                expect($dealJacketGroup->store_id)->toBe($storeId);
                expect($dealJacketGroup->completed)->toBeFalse();
            });
        });

        describe('Marking as Complete', function () {
            it('consultants cannot see Mark as Complete button when group has no deal jackets', function () {
                DealJacketGroup::factory()->create();

                $this->actingAs($this->consultant)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertDontSee('Mark as Complete');
            });

            it('consultants can see Mark as Complete button when group has deal jackets', function () {
                $dealJacketGroup = DealJacketGroup::factory()->create();
                App\Models\Dealer\Audit\DealJacket::factory()->create(['deal_jacket_group_id' => $dealJacketGroup->id]);

                $this->actingAs($this->consultant)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertSee('Mark as Complete');
            });

            it('consultants can successfully mark a group as complete', function () {
                $this->actingAs($this->consultant);
                $dealJacketGroup = DealJacketGroup::factory()->create();
                App\Models\Dealer\Audit\DealJacket::factory()->create(['deal_jacket_group_id' => $dealJacketGroup->id]);

                expect($dealJacketGroup->completed)->toBeFalse();

                Livewire::test(App\Http\Livewire\Tenant\Audit\DealJacket\Components\MarkCompleteModal::class, ['dealJacketGroupId' => $dealJacketGroup->id])
                    ->call('markComplete')
                    ->assertHasNoErrors();

                expect($dealJacketGroup->fresh()->completed)->toBeTrue();
            });
        });

        describe('Deleting Groups', function () {
            it('consultants can delete deal jacket group', function () {
                $this->actingAs($this->consultant);

                $dealJacketGroup = DealJacketGroup::factory()->create();

                Livewire::test(DealJacketGroupDeleteModal::class, ['dealJacketGroup' => $dealJacketGroup->id])
                    ->call('delete');

                expect(DealJacketGroup::count())->toBe(0);
            });
        });
    });

    describe('Manager Permissions', function () {
        describe('Page Access Restrictions', function () {
            it('managers cannot see Start Quarterly Audit button', function () {
                $this->actingAs($this->manager)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertStatus(200)
                    ->assertDontSee('Start Quarterly Audit');
            });
        });

        describe('Viewing Groups', function () {
            it('managers cannot see deal jacket group when not completed', function () {
                $dealJacketGroup = DealJacketGroup::factory()->create();

                $this->actingAs($this->manager)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertDontSee($dealJacketGroup->created_at);
            });

            it('managers can see deal jacket group when completed', function () {
                $dealJacketGroup = DealJacketGroup::factory()->create(['completed' => true]);

                $this->actingAs($this->manager)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertSee($dealJacketGroup->created_at->format('M d, Y'));
            });
        });

        describe('Creating Groups', function () {
            it('managers cannot create a new deal jacket group', function () {
                $this->actingAs($this->manager);
                $storeId = Store::first()->id;

                // Bind the current store to the app container (normally done by middleware)
                app()->instance('currentStore', $storeId);

                expect(DealJacketGroup::count())->toBe(0);

                Livewire::test(CreateNewGroupButton::class)
                    ->call('create');

                expect(DealJacketGroup::count())->toBe(0);
            });
        });

        describe('Deleting Groups', function () {
            it('managers cannot delete deal jacket group', function () {
                $this->actingAs($this->consultant);

                $dealJacketGroup = DealJacketGroup::factory()->create();

                Livewire::test(DealJacketGroupDeleteModal::class, ['dealJacketGroup' => $dealJacketGroup->id])
                    ->call('delete');

                expect(DealJacketGroup::count())->toBe(0);
            });
        });

        describe('Marking as Complete', function () {
            it('managers do not see Mark as Complete button when group is empty', function () {
                DealJacketGroup::factory()->create();

                $this->actingAs($this->manager)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertDontSee('Mark as Complete');
            });

            it('managers do not see Mark as Complete button even when group has deal jackets', function () {
                $dealJacketGroup = DealJacketGroup::factory()->create();
                App\Models\Dealer\Audit\DealJacket::factory()->create(['deal_jacket_group_id' => $dealJacketGroup->id]);

                $this->actingAs($this->manager)
                    ->get(route('dealer.audit.deal-jackets.index'))
                    ->assertDontSee('Mark as Complete');
            });

            it('managers cannot mark a group as complete even when attempting', function () {
                $this->actingAs($this->manager);
                $dealJacketGroup = DealJacketGroup::factory()->create();
                App\Models\Dealer\Audit\DealJacket::factory()->create(['deal_jacket_group_id' => $dealJacketGroup->id]);

                expect($dealJacketGroup->completed)->toBeFalse();

                Livewire::test(App\Http\Livewire\Tenant\Audit\DealJacket\Components\MarkCompleteModal::class, ['dealJacketGroupId' => $dealJacketGroup->id])
                    ->call('markComplete');

                expect($dealJacketGroup->fresh()->completed)->toBeFalse();
            });
        });
    });
});
