...
beforeEach(function () {
    ...
    $this->qualifiedIndividual = User::create([
...
describe('Document Upload', function () {
    it('allows submission with only a document uploaded', function () {
    ...
    it('validates document must be a PDF', function () {
    ...
    it('validates document size limit', function () {
    ...
    it('sends notification when document is uploaded', function () {
        $file = UploadedFile::fake()->create('risk-assessment.pdf', 1000, 'application/pdf');
...
describe('Form Submission Without Document', function () {
    it('requires all responses when no document is uploaded', function () {
        Livewire::test(NewForm::class, ['vid' => $this->vendorForm->id])
            ->call('submit')
            ->assertHasErrors(['data.*.response', 'signature']);
    });

    it('allows submission with completed form and signature', function () {
        $responses = [];
        for ($i = 1; $i <= 22; $i++) {
...
describe('Component Rendering', function () {
    ...
    it('redirects if form already has a signature', function () {
